...
class BB_Ajax_Advanced_Form {
	...
	public function __construct() {
		add_action( 'wp_ajax_bsb_process_advanced_form_submit', array( $this, 'process_ajax' ) );
		add_action( 'wp_ajax_nopriv_bsb_process_advanced_form_submit', array( $this, 'process_ajax' ) );
	}
	...
	public function process_ajax() {
		...
		if ( isset( $_POST['_bsb_form_id'] ) && ! empty( $_POST['_bsb_form_id'] ) && isset( $_POST['_bsb_form_post_id'] ) && ! empty( $_POST['_bsb_form_post_id'] ) ) {
			$this->start_buffer();

			if ( apply_filters( 'base_blocks_form_verify_nonce', false ) && ! check_ajax_referer( 'bsb_form_nonce', '_bsb_form_verify', false ) ) {
				$this->process_bail( __( 'Submission rejected, invalid security token. Reload the page and try again.', 'gutenam-blocks' ), __( 'Token invalid', 'gutenam-blocks' ) );
			}

			$post_id = sanitize_text_field( wp_unslash( $_POST['_bsb_form_post_id'] ) );

			$form_args = $this->get_form( $post_id );
			$messages  = $this->get_messages( $form_args['attributes'] );

			// Check Honey Pot.
			if ( isset( $form_args['attributes']['honeyPot'] ) && true === $form_args['attributes']['honeyPot'] && isset( $_POST['_bsb_verify_email'] ) ) {

				$honeypot_check = htmlspecialchars( $_POST['_bsb_verify_email'], ENT_QUOTES );
				if ( ! empty( $honeypot_check ) ) {
					$this->process_bail( __( 'Submission Rejected', 'gutenam-blocks' ), __( 'Spam Detected', 'gutenam-blocks' ) );
				}
			}
			...
			if ( isset( $form_args['attributes']['recaptcha'] ) && true === $form_args['attributes']['recaptcha'] ) {
				if ( isset( $form_args['attributes']['recaptchaVersion'] ) && 'v2' === $form_args['attributes']['recaptchaVersion'] ) {
					if ( ! isset( $_POST['g-recaptcha-response'] ) || empty( $_POST['g-recaptcha-response'] ) ) {
						$this->process_bail( $messages['recaptchaerror'], __( 'reCAPTCHA Failed', 'gutenam-blocks' ) );
					}
			...
			unset( $_POST['_bsb_form_sub_id'], $_POST['_bsb_verify_email'] );


			$responses = $this->process_fields( $form_args['fields'] );

			do_action( 'base_blocks_advanced_form_submission', $form_args, $form_args['fields'], $post_id, $responses );

			$this->after_submit_actions( $form_args, $responses, $post_id );

			$success  = apply_filters( 'base_blocks_advanced_form_submission_success', true, $form_args['fields'], $post_id, $responses );
			$messages = apply_filters( 'base_blocks_advanced_form_submission_messages', $messages );

			if ( ! $success ) {
				$this->process_bail( $messages['error'], __( 'Third Party Failed', 'gutenam-blocks' ) );
			} else {
				$final_data['html'] = '<div class="base-blocks-form-message base-blocks-form-success">' . $messages['success'] . '</div>';
				$this->send_json( $final_data );
			}

		} else {
			$this->process_bail( __( 'Submission failed', 'gutenam-blocks' ), __( 'No Data', 'gutenam-blocks' ) );
		}
	}
	...
	private function sanitize_field( $field_type, $value, $multi_select = false ) {
		switch ( $field_type ) {
			...
			case 'email':
				...
				break;
			case 'accept':
				$value = esc_html__( 'Accept', 'gutenam-blocks' );
				break;
			default:
				/**
				 ...
				 */
				$value = apply_filters( "base_blocks_form_sanitize_{$field_type}", $value );
		}
			...
			switch ( $action ) {
				...
				case 'redirect':
					if ( isset( $form_args['attributes']['redirect'] ) && ! empty( trim( $form_args['attributes']['redirect'] ) ) ) {
						self::$redirect = apply_filters( 'base_blocks_advanced_form_redirect', trim( $form_args['attributes']['redirect'] ), $form_args, $responses, $post_id );
					}
					break;
				case 'mailerlite':
	...
	public function process_fields( $fields ) {
		...
		foreach ( $fields as $index => $field ) {
			...
			$expected_field = 'bsb_field_' . $index;

			// Fail if required field is missing
			if ( empty( $_POST[ $expected_field ] ) && ! empty( $field['required'] ) && $field['required'] && $field['type'] !== 'file' ) {
				$required_message = ! empty( $field['required_message'] ) ? $field['required_message'] : __( 'Missing a required field', 'gutenam-blocks' );

				$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), $required_message );
			}
			...
			if ( $field['type'] === 'file' ) {
				...
				if ( $_FILES[ $expected_field ]['size'] > $max_upload_size_bytes ) {
					$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'File too large', 'gutenam-blocks' ) );
				}


				if ( ! is_uploaded_file( $_FILES[ $expected_field ]['tmp_name'] ) ) {
					$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'File was not uploaded', 'gutenam-blocks' ) );
				}
				...
				if ( ! in_array( $mime_type, $allowed_file_types ) ) {
					$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'File type not allowed', 'gutenam-blocks' ) );
				}
				...
				if ( $space < $file_size || upload_is_user_over_quota( false ) ) {
					$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'Not enough disk space on the server.', 'gutenam-blocks' ) );
				}

				$subfolder     = '/base_form/' . date( 'Y' ) . '/' . date( 'm' ) . '/';
				...
				if ( move_uploaded_file( $_FILES[ $expected_field ]['tmp_name'], $abs_file_path ) ) {
					...
					$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'Failed to upload file', 'gutenam-blocks' ) );
				}
			}
	...
	private function verify_recaptcha( $token ) {
		...
		$secret        = get_option( 'base_blocks_recaptcha_secret_key' );
		if ( ! $secret ) {
			return false;
		}
		$args           = array(
	...
	public function html_from_notices( $notices = array() ) {
		...
		foreach ( $notices as $note_type => $notice ) {
			if ( ! empty( $notice['note'] ) ) {
				$html .= '<div class="base-blocks-form-message base-blocks-form-warning">' . $notice['note'] . '</div>';
			}
		}
	...
	public function start_buffer() {
		if ( ! did_action( 'base_blocks_forms_buffer_started' ) ) {
			 ...
			 */
			do_action( 'base_blocks_forms_buffer_started' );
		}
	}
	...
	public function send_json( $data = array(), $is_error = false ) {
		$buffer = ob_get_clean();
		/**
		 * Runs before Base Blocks Forms returns json via wp_send_json() exposes output buffer
		 *
		 * @param string|null $buffer   Buffer contents
		 * @param bool        $is_error If we think this is an error response or not.
		 */
		do_action( 'base_blocks_forms_buffer_flushed', $buffer, $is_error );
	...
	private function set_permissions( $file ) {
		$permission = apply_filters( 'base_form_upload_permissions', 0644, $file );
	...
	private function get_form( $post_id ) {
		...
		if ( ! is_array( $blocks ) || empty( $blocks ) || ! isset( $blocks[0]['blockName'] ) || $blocks[0]['blockName'] !== 'base/advanced-form' || empty( $blocks[0]['innerBlocks'] ) ) {
			$this->process_bail( __( 'Submission Failed', 'gutenam-blocks' ), __( 'Data not found', 'gutenam-blocks' ) );
		}
		...
		foreach($post_meta as $meta_key => $meta_value ){
			if( strpos( $meta_key, '_bst_form_' ) === 0 && isset($meta_value[0]) ){
				$meta_args[ str_replace( '_bst_form_', '', $meta_key ) ] = maybe_unserialize($meta_value[0]);
			}
		}
		...
		foreach ( $blocks[0]['innerBlocks'] as $block ) {
			if ( ! is_object( $block ) && is_array( $block ) && isset( $block['blockName'] ) ) {
				...
				if ( strpos( $block['blockName'], 'base/advanced-form-' ) === 0 ) {
					$form_args['fields'][] = array_merge(
						$block['attrs'],
						array( 'type' => str_replace( 'base/advanced-form-', '', $block['blockName'] ) )
					);
				}
			}
		}
	...
	private function get_messages( $form_attributes ) {
		$messages = array(
			'success'        => esc_html__( 'Submission Success, Thanks for getting in touch!', 'gutenam-blocks' ),
			'error'          => esc_html__( 'Submission Failed', 'gutenam-blocks' ),
			'recaptchaerror' => esc_html__( 'Submission Failed, reCaptcha spam prevention. Please reload your page and try again.', 'gutenam-blocks' ),
		);
...
BB_Ajax_Advanced_Form::get_instance();
