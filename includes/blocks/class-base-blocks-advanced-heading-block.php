 ...
 * @category class
 */
class Base_Blocks_Advancedheading_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . ', .wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"]' );
		// Issue with span tag.
		...
		if ( ! empty( $attributes['icon'] ) ) {
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"]' );
			...
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] .bsb-adv-heading-icon svg' );
			$css->add_property( 'width', '1em');
			$css->add_property( 'height', '1em');
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] .bsb-adv-heading-icon' );
			...
			if ( ! empty( $attributes['iconColorHover'] ) ) {
				$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"]:hover .bsb-adv-heading-icon' );
		...
		// Highlight.
		$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . ' mark, .wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] mark' );
		...
		if ( ! empty( $attributes['linkColor'] ) ) {
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] a, .bst-adv-heading-link' . $unique_id . ', .bst-adv-heading-link' . $unique_id . ' .bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"]' );
			if ( ! empty( $attributes['linkColor'] ) ) {
				$css->add_property( 'color', $css->render_color( $attributes['linkColor'] ) );
			}
		}
		if ( ! empty( $attributes['linkHoverColor'] ) ) {
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] a:hover, .bst-adv-heading-link' . $unique_id . ':hover, .bst-adv-heading-link' . $unique_id . ':hover .bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"]' );
			$css->add_property( 'color', $css->render_color( $attributes['linkHoverColor'] ) );
		}
		if ( ! empty( $attributes['linkStyle'] ) ) {
			$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] a, a.bsb-advanced-heading-link.bst-adv-heading-link' . $unique_id );
			if ( 'none' === $attributes['linkStyle'] ) {
				$css->add_property( 'text-decoration', 'none' );
			} else if ( $attributes['linkStyle'] === 'underline' ) {
				$css->add_property( 'text-decoration', 'underline' );
				$css->set_selector( '.wp-block-base-advancedheading.bst-adv-heading' . $unique_id . '[data-bsb-block="bsb-adv-heading' . $unique_id . '"] a:hover, a.bsb-advanced-heading-link.bst-adv-heading-link' . $unique_id . ':hover' );
				$css->add_property( 'text-decoration', 'underline' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		if ( strpos( $content, 'bst-typed-text') !== false ) {
			$this->enqueue_script( 'base-blocks-' . $this->block_name );
		}
		if ( ! empty( $attributes['icon'] ) ) {
			...
			// Start empty.
			$content = '';
			$reveal_animation = ( ! empty( $attributes['baseAnimation'] ) && ( 'reveal-left' === $attributes['baseAnimation'] || 'reveal-right' === $attributes['baseAnimation'] || 'reveal-up' === $attributes['baseAnimation'] || 'reveal-down' === $attributes['baseAnimation'] ) ? true : false );
			$wrapper = $reveal_animation ? true : false;
			$icon_side = ! empty( $attributes['iconSide'] ) ? $attributes['iconSide'] : 'left';
			$classes = array( 'bst-adv-heading' . $unique_id, 'wp-block-base-advancedheading', 'bst-adv-heading-has-icon' );
			...
			$content_args = array(
				...
				'data-bsb-block' => 'bsb-adv-heading' . $unique_id,
			);
			if ( ! empty( $attributes['anchor'] ) ) {
				$content_args['id'] = $attributes['anchor'];
			}
			$content_args = base_apply_aos_wrapper_args( $attributes, $content_args );
			...
			$content = sprintf( '<%1$s %2$s>%3$s<span class="bsb-adv-text-inner">%4$s</span>%5$s</%1$s>', $tag_name, $inner_content_attributes, $icon_left, $text_content, $icon_right );
			if ( ! empty( $attributes['link'] ) ) {
				$link_classes = array( 'bsb-advanced-heading-link', 'bst-adv-heading-link' . $unique_id );
				if ( ! empty( $attributes['linkStyle'] ) ) {
			...
			if ( $wrapper ) {
				$wrapper_classes = array( 'bsb-adv-heading-wrap' . $unique_id, 'base-advanced-heading-wrapper' );
				if ( $reveal_animation ) {
					$wrapper_classes[] = 'base-heading-clip-animation';
				}
				if ( ! empty( $attributes['class'] ) && $wrapper ) {
	...
	public function register_scripts() {
		...
		wp_register_style( 'base-blocks-' . $this->block_name, false );
		$heading_css = '.wp-block-base-advancedheading mark{background:transparent;border-style:solid;border-width:0}.wp-block-base-advancedheading mark.bst-highlight{color:#f76a0c;}.bsb-adv-heading-icon{display: inline-flex;justify-content: center;align-items: center;}';
		// Short term fix for an issue with heading wrapping.
		if ( class_exists( '\Base\Theme' ) ) {
			$heading_css .= '.single-content .base-advanced-heading-wrapper h1, .single-content .base-advanced-heading-wrapper h2, .single-content .base-advanced-heading-wrapper h3, .single-content .base-advanced-heading-wrapper h4, .single-content .base-advanced-heading-wrapper h5, .single-content .base-advanced-heading-wrapper h6 {margin: 1.5em 0 .5em;}.single-content .base-advanced-heading-wrapper+* { margin-top:0;}';
		}
		wp_add_inline_style( 'base-blocks-' . $this->block_name, $heading_css );
		wp_register_script( 'base-blocks-typed-js', BASE_BLOCKS_URL . 'includes/assets/js/typed.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-' . $this->block_name, BASE_BLOCKS_URL . 'includes/assets/js/bsb-advanced-heading.min.js', array( 'base-blocks-typed-js' ), GUTENAM_BLOCKS_VERSION, true );
	}
	/**
	...
	private function get_icon( $attributes ) {
		...
		return '<span class="bsb-svg-icon-wrap bsb-adv-heading-icon bsb-svg-icon-' . esc_attr( $attributes['icon'] ) . ' bsb-adv-heading-icon-side-' . esc_attr( $icon_side ) . '">' . $svg_icon . '</span>';
	}
...
Base_Blocks_Advancedheading_Block::get_instance();
