 ...
 * @category class
 */
class Base_Blocks_Testimonial_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$content = '';
		if ( $layout === 'carousel' ) {
			$content .= '<div class="bst-blocks-testimonial-carousel-item bsb-slide-item">';
		}

		$content .= '<div class="bst-testimonial-item-wrap bst-testimonial-item-' . $unique_id . ' ' . ( $containerVAlign ? "testimonial-valign-" . $containerVAlign : "" ) . '">';
		$content .= '<div class="bst-testimonial-text-wrap">';
		...
		// end: bst-testimonial-text-wrap
		$content .= '</div>';
		if ( ( $displayMedia && ! in_array( $style, array(
					'card',
					'inlineimage'
				) ) ) || $displayOccupation || $displayName ) {
			$content .= '<div class="bst-testimonial-meta-wrap">';
			if ( $displayMedia && !in_array( $style, array(
					'card',
					'inlineimage'
			...
			$content .= '<div class="bst-testimonial-meta-name-wrap">';

			if ( $displayName ) {
				$content .= '<div class="bst-testimonial-name-wrap">';
				$content .= '<div class="bst-testimonial-name">'. $attributes['name'] .'</div>';
				$content .= '</div>';
			}

			if ( $displayOccupation ) {
				$content .= '<div class="bst-testimonial-occupation-wrap">';
					$content .= '<div class="bst-testimonial-occupation">'. $attributes['occupation'] .'</div>';
				$content .= '</div>';
			}

			$content .= '</div>'; // end: bst-testimonial-meta-name-wrap
			$content .= '</div>'; // end: bst-testimonial-meta-wrap
		}
	...
	private function get_context( $context, $key, $default = '' ) {
		return isset( $context['base/testimonials-' . $key] ) ? $context['base/testimonials-' . $key] : $default;
	}

	private function render_icon( $attributes, $iconStyles ) {
		$icon = '<div class="bst-svg-testimonial-global-icon-wrap">';
		...
		$svg = Base_Blocks_Svg_Render::render( $iconStyles[ 0 ]['icon'], $fill, $stroke_width, $title );

		$icon .= "<div class='bst-svg-testimonial-global-icon bst-svg-testimonial-global-icon-icon-" . $iconStyles[ 0 ]['icon'] . "'>";
		$icon .= $svg;
		$icon .= "</div>";
	...
	private function render_media( $attributes, $mediaStyles, $style, $containerMaxWidth ) {
		...
		$media = '<div class="bst-testimonial-media-wrap">';
		$media .= '<div class="bst-testimonial-media-inner-wrap">';
		$media .= '<div class="base-testimonial-image-intrisic">';
		...
		if( $attributes['media'] === 'icon' && $attributes['icon']) {
			...
			$svg = Base_Blocks_Svg_Render::render( $attributes['icon'], $fill, $stroke_width, $attributes['ititle'], false, $extras);

			$media .= '<div class="bst-svg-testimonial-icon bst-svg-testimonial-icon-'. $attributes['icon'] .'">';
			$media .= $svg;
			$media .= '</div>';
		}
		if( $attributes['media'] !== 'icon' && $attributes['url'] ) {
			$media .= '<div class="bst-testimonial-image" style="background-image: url(' . $urlOutput . ');' . ( $style === 'card' ? 'background-size: '. $mediaStyles[0]['backgroundSize'] . ';' : '' ) . '' . ( isset( $mediaStyles[ 0 ]['borderRadius'] ) && is_numeric( $mediaStyles[ 0 ]['borderRadius'] ) ? 'border-radius: ' . $mediaStyles[ 0 ]['borderRadius'] . 'px;' : '' ) . '"></div>';
		}
		$media .= '</div>';
		$media .= '</div>';
		$media .= '</div>';
	...
	private function render_title( $attributes, $titleFont ) {
		$title = '<div class="bst-testimonial-title-wrap">';

		$title .= '<h'. $titleFont[0]['level'] .' class="bst-testimonial-title">';
	...
	private function render_rating( $attributes, $ratingStyles ) {
		...
		$svg_title = sprintf( esc_html__( '%d star rating', 'gutenam-blocks' ), $attributes['rating'] );
		$svg = Base_Blocks_Svg_Render::render( 'fas_star', 'currentColor', ( ! empty(  $ratingStyles[ 0 ]['size'] ) ? $ratingStyles[ 0 ]['size'] : '' ), $svg_title, false, $extras );

		$rating = '<div class="bst-testimonial-rating-wrap bst-testimonial-rating-'. $attributes['rating'] .'">';

		for( $i = 0; $i < $attributes['rating']; $i++ ) {
			$rating .= '<div class="bst-svg-testimonial-rating-icon bst-svg-testimonial-rating-icon-'. ($i + 1) .'">';
			$rating .= $svg;
			$rating .= '</div>';
		}
	...
	private function render_content( $attributes ) {
		$content = '<div class="bst-testimonial-content-wrap">';
		$content .= '<div class="bst-testimonial-content">';
...
Base_Blocks_Testimonial_Block::get_instance();
