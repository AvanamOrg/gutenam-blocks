 ...
 * @category class
 */
class Base_Blocks_Posts_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.bsb-posts-id-' . $unique_id . ' .entry.loop-entry .entry-header .entry-title' );
		...
		if ( isset( $attributes['loopStyle'] ) && 'unboxed' === $attributes['loopStyle'] ) {
			if ( class_exists( 'Base\Theme' ) ) {
				$css->set_selector( '.bsb-posts-id-' . $unique_id . ' .loop-entry' );
				$css->add_property( 'background', 'transparent' );
				$css->add_property( 'box-shadow', 'none' );
				$css->set_selector( '.bsb-posts-id-' . $unique_id . ' .loop-entry > .entry-content-wrap' );
				$css->add_property( 'padding', '0px' );
				$css->set_selector( '.bsb-posts-id-' . $unique_id . ' .loop-entry .post-thumbnail' );
				$css->add_property( 'margin-bottom', '1em' );
			}
		} else {
			if ( class_exists( 'Base\Theme' ) && defined( 'BASE_VERSION' ) ) {
				if ( version_compare( BASE_VERSION, '1.0.16', '<' ) ) {
					$css->set_selector( '.wp-block-base-posts.bsb-posts-id-' . $unique_id . ' .entry.loop-entry > .entry-content-wrap' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		global $base_blocks_posts_not_in;
		if ( ! isset( $base_blocks_posts_not_in ) || ! is_array( $base_blocks_posts_not_in ) ) {
			$base_blocks_posts_not_in = array();
		}
		// CSS classes.
		$classes   = array();
		$classes[] = 'wp-block-base-posts';
		$classes[] = 'bsb-posts';
		$classes[] = 'bsb-posts-id-' . ( isset( $attributes['uniqueID'] ) ? $attributes['uniqueID'] : '' );
		$classes[] = 'content-wrap';
		$classes[] = 'grid-cols';
		$classes[] = 'bsb-posts-style-' . ( isset( $attributes['loopStyle'] ) ? $attributes['loopStyle'] : 'boxed' );
		...
		$classes = apply_filters( 'base_blocks_posts_container_classes', $classes );
		do_action( 'base_blocks_posts_before_query', $attributes );
		if ( apply_filters( 'base_blocks_posts_block_exclude_current', true ) && is_singular() ) {
			if ( ! in_array( get_the_ID(), $base_blocks_posts_not_in, true ) ) {
				$base_blocks_posts_not_in[] = get_the_ID();
			}
		}
		...
		$args      = array(
			...
			'post__not_in'        => ( isset( $base_blocks_posts_not_in ) && is_array( $base_blocks_posts_not_in ) ? $base_blocks_posts_not_in : array() ),
		);
		...
		$args = apply_filters( 'base_blocks_posts_query_args', $args );
		$loop = new WP_Query( $args );
		ob_start();

		do_action( 'base_blocks_posts_query_start', $attributes, $loop );
		if ( $loop->have_posts() ) {
			...
			while ( $loop->have_posts() ) {
				...
				base_blocks_get_template( 'entry.php', array( 'attributes' => $attributes ) );
			}
			echo '</div>';
		} else {
			echo wp_kses_post( apply_filters( 'base_blocks_posts_empty_query', '<p>' . esc_html__( 'No posts', 'gutenam-blocks' ) . '</p>' ) );
		}
		wp_reset_postdata();
		do_action( 'base_blocks_posts_query_end', $attributes, $loop );
		do_action( 'base_blocks_posts_after_query', $attributes );
...
Base_Blocks_Posts_Block::get_instance();
