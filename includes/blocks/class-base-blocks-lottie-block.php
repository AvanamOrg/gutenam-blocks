 ...
 * @category class
 */
class Base_Blocks_Lottie_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.bsb-lottie-container' . $unique_id );
		...
		if ( isset( $attributes['useRatio'] ) && $attributes['useRatio'] ) {
			...
			$css->set_selector( '.bsb-lottie-container' . $unique_id . ' .bsb-is-ratio-animation' );
			$css->add_property( 'padding-bottom', isset( $attributes['ratio'] ) && is_numeric( $attributes['ratio'] ) ? $attributes['ratio'] . '%' : '100%' );
		}

		if ( ! isset( $attributes['useRatio'] ) || ! $attributes['useRatio'] ) {
			$css->set_selector( '.bsb-lottie-container' . $unique_id . ' dotlottie-player' );
			$css->add_property( 'max-width', $width );
		}
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		if ( ! wp_script_is( 'base-blocks-dotlottie-player-js', 'enqueued' ) ) {
			wp_enqueue_script( 'base-blocks-dotlottie-player-js' );
		}

		if ( isset( $attributes['uniqueID'] ) ) {
			$unique_id              = $attributes['uniqueID'];
			$player_style_id        = 'bsb-lottie-player' . esc_attr( $unique_id );
			$player_simple_style_id = str_replace( array( '-' ), '', $player_style_id );
			$style_id               = 'bsb-lottie' . esc_attr( $unique_id );

			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'base_blocks_render_inline_css', true, 'lottie', $unique_id ) ) {
				// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
				$attributes = apply_filters( 'base_blocks_lottie_render_block_attributes', $attributes );
			}
			...
			if ( isset( $attributes['onlyPlayOnScroll'] ) && $attributes['onlyPlayOnScroll'] === true || isset( $attributes['waitUntilInView'] ) && $attributes['waitUntilInView'] === true ) {
				if ( ! wp_script_is( 'base-blocks-lottieinteractivity-js', 'enqueued' ) ) {
					wp_enqueue_script( 'base-blocks-lottieinteractivity-js' );
				}
			...
			$content .= '<div class="bsb-lottie-container bsb-lottie-container' . esc_attr( $unique_id ) . ' '. $align .'">';
				if ( isset( $attributes['useRatio'] ) && $attributes['useRatio'] ) {
					$content .= '<div class="bsb-is-ratio-animation">';
				}
				...
				$content .= 'src="' . $this->getAnimationUrl( $attributes ) . '"
							id="bsb-lottie-player' . $unique_id .'"
						></dotlottie-player>';
				if ( isset( $attributes['useRatio'] ) && $attributes['useRatio'] ) {
					$content .= '</div>';
				}
	...
	public function getAnimationUrl( $attributes ) {
		...
		if ( $attributes['fileSrc'] === 'url' ) {
			...
			$url = $rest_url . 'bsb-lottieanimation/v1/animations/' . $attributes['localFile'][0]['value'] . '.json';
		}

		if ( $url === '' || $url === $rest_url . 'bsb-lottieanimation/v1/animations/' ) {
	...
	public function register_scripts() {
		...
		wp_register_script( 'base-blocks-lottieinteractivity-js', BASE_BLOCKS_URL . 'includes/assets/js/lottie-interactivity.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-dotlottie-player-js', BASE_BLOCKS_URL . 'includes/assets/js/lottie-dotlottie.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
	}
}

Base_Blocks_Lottie_Block::get_instance();
