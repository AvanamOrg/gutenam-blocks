 ...
 * @category class
 */
class Base_Blocks_Abstract_Block {
	...
	public function __construct() {
		...
		add_filter( 'base_blocks_blocks_to_generate_post_css', array( $this, 'add_block_to_post_generate_css' ) );
	}
	...
	public function on_init() {
		register_block_type(
			BASE_BLOCKS_PATH . 'dist/blocks/' . $this->block_name . '/block.json',
			array(
				'render_callback' => array( $this, 'render_css' ),
				'editor_script'   => 'base-blocks-' . $this->block_name,
				'editor_style'    => 'base-blocks-' . $this->block_name,
			)
		);
	}
	...
	public function add_block_to_post_generate_css( $block_class_array ) {
		if ( ! isset( $block_class_array[ $this->namespace . '/' . $this->block_name ] ) ) {
			$block_class_array[ $this->namespace . '/' . $this->block_name ] = 'Base_Blocks_' . str_replace( ' ', '_', ucwords( str_replace( '-', ' ', $this->block_name ) ) ) . '_Block';
		}
	...
	public function should_render_inline_stylesheet( $name ) {
		if ( apply_filters( 'base_blocks_force_render_inline_stylesheet', false, $name ) || ( ! is_admin() && ! wp_style_is( $name, 'done' ) && ! is_feed() ) ) {
	...
	public function should_render_inline( $name, $unique_id ) {
		if ( ( doing_filter( 'the_content' ) && ! is_feed() ) || apply_filters( 'base_blocks_force_render_inline_css_in_content', false, $name, $unique_id ) || is_customize_preview() ) {
			return true;
		}
	...
	public function output_head_data( $block ) {
		if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
			...
			if ( isset( $attributes['uniqueID'] ) ) {
				...
				if ( ! $css_class->has_styles( 'bsb-' . $this->block_name . $unique_id ) && apply_filters( 'base_blocks_render_head_css', true, $this->block_name, $attributes ) ) {
					// Filter attributes for easier dynamic css.
					$attributes = apply_filters( 'base_blocks_' . $this->block_name . '_render_block_attributes', $attributes );
	...
	public function render_scripts( $attributes, $inline = false ) {
		...
		if ( $this->has_script ) {
			if ( ! wp_script_is( 'base-blocks-' . $this->block_name, 'enqueued' ) ) {
	...
	public function render_css( $attributes, $content, $block_instance ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			$unique_id = $attributes['uniqueID'];
			$unique_style_id = apply_filters( 'base_blocks_build_render_unique_id', $attributes['uniqueID'], $this->block_name, $attributes );
			$css_class = Base_Blocks_CSS::get_instance();

			// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
			$attributes = apply_filters( 'base_blocks_' . str_replace( '-', '_', $this->block_name ) . '_render_block_attributes', $attributes );

			$content   = $this->build_html( $attributes, $unique_id, $content, $block_instance );
			if ( ! $css_class->has_styles( 'bsb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'base_blocks_render_inline_css', true, $this->block_name, $unique_id ) ) {
			...
			} elseif ( ! wp_is_block_theme() && ! $css_class->has_header_styles( 'bsb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'base_blocks_render_inline_css', true, $this->block_name, $unique_id ) ) {
	...
	public function register_scripts() {
		...
		wp_register_style( 'base-blocks-' . $this->block_name, BASE_BLOCKS_URL . 'dist/style-blocks-' . $this->block_name . '.css', array(), GUTENAM_BLOCKS_VERSION );
	}
