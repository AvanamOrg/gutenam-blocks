 ...
 * @category class
 */
class Base_Blocks_Rowlayout_Block extends Base_Blocks_Abstract_Block {
	/**
	...
	public function render_scripts( $attributes, $inline = false ) {
		...
		if ( ( isset( $attributes['bgImg'] ) && ! empty( $attributes['bgImg'] ) && isset( $attributes['bgImgAttachment'] ) && 'parallax' === $attributes['bgImgAttachment'] ) || ( isset( $attributes['overlayBgImg'] ) && ! empty( $attributes['overlayBgImg'] ) && isset( $attributes['overlayBgImgAttachment'] ) && 'parallax' === $attributes['overlayBgImgAttachment'] ) ) {
			$this->enqueue_script( 'base-blocks-parallax-js' );
		}
		...
		if ( isset( $attributes['backgroundSettingTab'] ) && 'video' === $attributes['backgroundSettingTab'] && isset( $attributes['backgroundVideo'] ) && isset( $attributes['backgroundVideo'][0] ) && isset( $attributes['backgroundVideo'][0]['btns'] ) && true === $attributes['backgroundVideo'][0]['btns'] ) {
			$this->enqueue_script( 'base-blocks-video-bg' );
		}
	}
	/**
	 * Render for block scripts block.
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		if ( ! empty( $attributes['bbVersion'] ) && $attributes['bbVersion'] > 1 ) {
			$margin_selector = '.bsb-row-layout-wrap.wp-block-base-rowlayout.bsb-row-layout-id' . $unique_id;
			$base_selector = '.bsb-row-layout-id' . $unique_id;
			$inner_selector = '.bsb-row-layout-id' . $unique_id . ' > .bst-row-column-wrap';
		} else {
			$margin_selector = '#bst-layout-id' . $unique_id;
			$base_selector = '#bst-layout-id' . $unique_id;
			$inner_selector = '#bst-layout-id' . $unique_id . ' > .bst-row-column-wrap';
		}
		...
		if ( ! empty( $attributes['verticalAlignment'] ) ) {
			switch ( $attributes['verticalAlignment'] ) {
				case 'middle':
					...
					$css->set_selector( ':where(' . $inner_selector . ') > .wp-block-base-column' );
		...
		if ( ! empty( $attributes['collapseOrder'] ) && 'left-to-right' !== $attributes['collapseOrder'] && in_array( $layout, $collapse_layouts ) ) {
			foreach ( range( 1, $columns ) as $item_count ) {
				$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(' . $item_count . ')' );
				$css->add_property( 'order', ( $columns - $item_count + 1 ) );
			}
			// Row Two.
			foreach ( range( 1, $columns ) as $item_count ) {
				$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(' . ( $item_count + $columns ) . ')' );
				$css->add_property( 'order', ( $columns - $item_count + 11 ) );
			}
			// Row Three.
			foreach ( range( 1, $columns ) as $item_count ) {
				$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(' . ( $item_count + $columns + $columns ) . ')' );
				$css->add_property( 'order', ( $columns - $item_count + 21 ) );
			}
			// Row Four.
			foreach ( range( 1, $columns ) as $item_count ) {
				$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(' . ( $item_count + $columns + $columns + $columns ) . ')' );
		...
		if ( isset( $attributes['inheritMaxWidth'] ) && true === $attributes['inheritMaxWidth'] && ! empty( $attributes['align'] ) && 'full' === $attributes['align'] && 2 === $columns && 'row' !== $layout && ( ( isset( $attributes['breakoutLeft'] ) && true === $attributes['breakoutLeft'] ) || ( isset( $attributes['breakoutRight'] ) && true === $attributes['breakoutRight'] ) ) ) {
			...
			if ( apply_filters( 'base_blocks_css_output_media_queries', true ) ) {
				if ( isset( $content_width ) && ! empty( $content_width ) ) {
					...
					if ( ( isset( $attributes['breakoutLeft'] ) && true === $attributes['breakoutLeft'] ) ) {
						$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(1):not(.specificity)' );
						$css->add_property( 'margin-left', 'var(--breakout-negative-margin-left)' );
					}
					if ( ( isset( $attributes['breakoutRight'] ) && true === $attributes['breakoutRight'] ) ) {
						$css->set_selector( $inner_selector . ' > .wp-block-base-column:nth-child(2):not(.specificity)' );
			...
			if ( ( isset( $attributes['breakoutLeft'] ) && true === $attributes['breakoutLeft'] ) ) {
				$css->set_selector( $inner_selector . ':not(.bst-tab-layout-row) > .wp-block-base-column:nth-child(1)' );
				$css->add_property( 'margin-left', 'calc( ' . $padding_left . ' *-1 )' );
			}
			if ( ( isset( $attributes['breakoutRight'] ) && true === $attributes['breakoutRight'] ) ) {
				$css->set_selector( $inner_selector . ':not(.bst-tab-layout-row) > .wp-block-base-column:nth-child(2)' );
				$css->add_property( 'margin-right', 'calc( ' . $padding_right . ' *-1 )' );
			}
			$css->set_media_state( 'desktop' );
		}
		...
		if ( isset( $tablet_background['enable'] ) && $tablet_background['enable'] ) {
			...
			if ( 'normal' !== $background_type ) {
				$css->set_selector( $base_selector . ' .bsb-blocks-bg-video-container,' . $base_selector . ' .bsb-blocks-bg-slider' );
		...
		if ( ! empty( $attributes['overlay'] ) || ! empty( $attributes['overlayBgImg'] ) || ! empty( $attributes['overlaySecond'] ) || ! empty( $attributes['overlayGradient'] ) ) {
			$css->set_selector( $base_selector . ' > .bst-row-layout-overlay' );
		...
		if ( isset( $attributes['bottomSep'] ) && 'none' != $attributes['bottomSep'] ) {
			...
			$css->set_selector( $base_selector . ' .bst-row-layout-bottom-sep' );
			$css->add_property( 'height', $bottomSepHeight . $bottomSepUnit );

				if ( isset( $attributes['bottomSepWidth'] ) ) {
					$css->set_selector( $base_selector . ' .bst-row-layout-bottom-sep svg' );
		...
		if ( isset( $attributes['topSep'] ) && 'none' != $attributes['topSep'] ) {
			...
			$topSepHeight = !empty ($attributes['topSepHeight']) ? $attributes['topSepHeight'] : '100';
				$css->set_selector( $base_selector . ' .bst-row-layout-top-sep' );
				$css->add_property( 'height', $topSepHeight . $topSepUnit );
			if ( $css->is_number( $attributes['topSepWidth'] ) ) {
				$css->set_selector( $base_selector . ' .bst-row-layout-top-sep svg' );
		...
		$css->set_media_state( 'desktop' );
		if ( isset( $attributes['baseBlockCSS'] ) && ! empty( $attributes['baseBlockCSS'] ) ) {
			$css->add_css_string( str_replace( 'selector', $base_selector, $attributes['baseBlockCSS'] ) );
		}
	...
	public function get_divider_render( $divider, $location ) {
		...
		$paths = apply_filters( 'base_blocks_row_divider_paths', $paths, $location );
		$output = '';
		if ( isset( $paths[ $divider ] ) ) {
			$output .= '<div class="bst-row-layout-' . esc_attr( $location ) . '-sep bst-row-sep-type-' . esc_attr( $divider ) . '">';
	...
	public function output_color( $color, $opacity = null ) {
		...
		if ( ! is_array( $color ) && strpos( $color, 'palette' ) === 0 ) {
			...
			$color = base_blocks_hex2rgba( $color, $opacity );
		}
		return $color;
	}
	/**
	...
	public function get_slider_render( $attributes ) {
		...
		$output .= '<div class="bst-blocks-carousel bsb-blocks-bg-slider bst-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '">';
		$output .= '<div class="bst-blocks-carousel-init bsb-blocks-bg-slider-init bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . '" data-slider-anim-speed="' . esc_attr( $tran_speed ) . '" data-slider-type="slider" data-slider-scroll="1" data-slider-arrows="' . ( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-fade="' . ( $fade ? 'true' : 'false' ) . '" data-slider-dots="' . ( 'none' === $dot_style ? 'false' : 'true' ) . '" data-slider-hover-pause="false" data-slider-auto="' . ( $auto ? 'true' : 'false' ) . '" data-slider-speed="' . esc_attr( $speed ) . '">';
		$item = 1;
		foreach ( $attributes['backgroundSlider'] as $key => $slide ) {
			...
			$output .= '<div class="bsb-bg-slide-contain">';
			$output .= '<div class="bsb-bg-slide bsb-bg-slide-' . esc_attr( $key ) . '" style="' . esc_attr( implode( ' ', $style_output ) ) . '">';
			$output .= '</div>';
			$output .= '</div>';
	...
	public function prevent_preload_when_hidden( $attributes ) {
		if ( ( ( ! empty( $attributes['tabletBackground'][0]['enable'] ) && true == $attributes['tabletBackground'][0]['enable'] ) || ( ! empty( $attributes['mobileBackground'][0]['enable'] ) && true == $attributes['mobileBackground'][0]['enable'] ) ) && apply_filters( 'base_blocks_rowlayout_prevent_preload_for_mobile', true ) ) {
			return true;
		}
		return false;
	}
	...
	public function get_video_render( $attributes ) {
		...
		if ( isset( $video_attributes['btns'] ) && $video_attributes['btns'] ) {
			$btns_output .= '<div class="bsb-background-video-buttons-wrapper bsb-background-video-buttons-html5">';
			$btns_output .= '<button class="bsb-background-video-play bsb-toggle-video-btn" aria-label="' . __( 'Play', 'gutenam-blocks' ) . '" aria-hidden="true" style="display: none;"><svg viewBox="0 0 448 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M424.4 214.7L72.4 6.6C43.8-10.3 0 6.1 0 47.9V464c0 37.5 40.7 60.1 72.4 41.3l352-208c31.4-18.5 31.5-64.1 0-82.6z"></path></svg></button>';
			$btns_output .= '<button class="bsb-background-video-pause bsb-toggle-video-btn" aria-label="' . __( 'Pause', 'gutenam-blocks' ) . '" aria-hidden="false"><svg viewBox="0 0 448 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M144 479H48c-26.5 0-48-21.5-48-48V79c0-26.5 21.5-48 48-48h96c26.5 0 48 21.5 48 48v352c0 26.5-21.5 48-48 48zm304-48V79c0-26.5-21.5-48-48-48h-96c-26.5 0-48 21.5-48 48v352c0 26.5 21.5 48 48 48h96c26.5 0 48-21.5 48-48z"></path></svg></button>';
			if ( isset( $video_attributes['mute'] ) && false === $video_attributes['mute'] ) {
				$btns_output .= '<button class="bsb-background-video-unmute bsb-toggle-video-btn" aria-label="' . __( 'Unmute', 'gutenam-blocks' ) . '" aria-hidden="true" style="display: none;"><svg viewBox="0 0 256 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M256 88.017v335.964c0 21.438-25.943 31.998-40.971 16.971L126.059 352H24c-13.255 0-24-10.745-24-24V184c0-13.255 10.745-24 24-24h102.059l88.971-88.954c15.01-15.01 40.97-4.49 40.97 16.971z"></path></svg></button>';
				$btns_output .= '<button class="bsb-background-video-mute bsb-toggle-video-btn" aria-label="' . __( 'Mute', 'gutenam-blocks' ) . '" aria-hidden="false"><svg viewBox="0 0 576 512" height="16" width="16" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M256 88.017v335.964c0 21.438-25.943 31.998-40.971 16.971L126.059 352H24c-13.255 0-24-10.745-24-24V184c0-13.255 10.745-24 24-24h102.059l88.971-88.954c15.01-15.01 40.97-4.49 40.97 16.971zm182.056-77.876C422.982.92 403.283 5.668 394.061 20.745c-9.221 15.077-4.473 34.774 10.604 43.995C468.967 104.063 512 174.983 512 256c0 73.431-36.077 142.292-96.507 184.206-14.522 10.072-18.129 30.01-8.057 44.532 10.076 14.528 30.016 18.126 44.531 8.057C529.633 438.927 576 350.406 576 256c0-103.244-54.579-194.877-137.944-245.859zM480 256c0-68.547-36.15-129.777-91.957-163.901-15.076-9.22-34.774-4.471-43.994 10.607-9.22 15.078-4.471 34.774 10.607 43.994C393.067 170.188 416 211.048 416 256c0 41.964-20.62 81.319-55.158 105.276-14.521 10.073-18.128 30.01-8.056 44.532 6.216 8.96 16.185 13.765 26.322 13.765a31.862 31.862 0 0 0 18.21-5.709C449.091 377.953 480 318.938 480 256zm-96 0c0-33.717-17.186-64.35-45.972-81.944-15.079-9.214-34.775-4.463-43.992 10.616s-4.464 34.775 10.615 43.992C314.263 234.538 320 244.757 320 256a32.056 32.056 0 0 1-13.802 26.332c-14.524 10.069-18.136 30.006-8.067 44.53 10.07 14.525 30.008 18.136 44.53 8.067C368.546 316.983 384 287.478 384 256z"></path></svg></button>';
			}
			$btns_output .= '</div>';
		}

		$output = sprintf( '<div class="bsb-blocks-bg-video-container"><video %1$s></video>%2$s</div>', implode( ' ', $video_html_attributes ), $btns_output );
		return $output;
	}
	/**
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		if ( ! empty( $attributes['bbVersion'] ) && $attributes['bbVersion'] > 1 ) {
			...
			$outer_classes = array( 'bsb-row-layout-wrap', 'bsb-row-layout-id' . $unique_id );
			$outer_classes[] = ! empty( $attributes['align'] ) ? 'align' . $attributes['align'] : 'alignnone';
			if ( isset( $attributes['vsdesk'] ) && $attributes['vsdesk'] ) {
				$outer_classes[] = 'bsb-v-lg-hidden';
			}
			if ( isset( $attributes['vstablet'] ) && $attributes['vstablet'] ) {
				$outer_classes[] = 'bsb-v-md-hidden';
			}
			if ( isset( $attributes['vsmobile'] ) && $attributes['vsmobile'] ) {
				$outer_classes[] = 'bsb-v-sm-hidden';
			}
			if ( ! empty( $attributes['bgColorClass'] ) ) {
			...
			if ( ! empty( $attributes['bgImg'] ) || ! empty( $attributes['bgColor'] ) || ! empty( $attributes['gradient'] ) || ! empty( $attributes['overlay'] ) || ! empty( $attributes['overlayBgImg'] ) || ! empty( $attributes['overlayGradient'] ) ) {
				$outer_classes[] = 'bst-row-has-bg';
			}
			if ( ! empty( $attributes['bgImg'] ) && ! empty( $attributes['bgImgAttachment'] ) && 'parallax' === $attributes['bgImgAttachment'] ) {
				$outer_classes[] = 'bst-jarallax';
			}
			$inner_classes = array( 'bst-row-column-wrap' );
			$inner_classes[] = ! empty( $attributes['columns'] ) ? 'bst-has-' . $attributes['columns'] . '-columns' : 'bst-has-2-columns';
			$inner_classes[] = ! empty( $attributes['colLayout'] ) ? 'bst-row-layout-' . $attributes['colLayout'] : 'bst-row-layout-equal';
			$inner_classes[] = ! empty( $attributes['tabletLayout'] ) ? 'bst-tab-layout-' . $attributes['tabletLayout'] : 'bst-tab-layout-inherit';
			$inner_classes[] = ! empty( $attributes['mobileLayout'] ) ? 'bst-mobile-layout-' . $attributes['mobileLayout'] : 'bst-mobile-layout-inherit';

			if ( ! empty( $attributes['verticalAlignment'] ) ) {
				$inner_classes[] = 'bst-row-valign-' . $attributes['verticalAlignment'];
			}
			if ( isset( $attributes['columnsInnerHeight'] ) && $attributes['columnsInnerHeight'] ) {
				$inner_classes[] = 'bst-inner-column-height-full';
			}
			if ( isset( $attributes['inheritMaxWidth'] ) && $attributes['inheritMaxWidth'] ) {
				$inner_classes[] = 'bsb-theme-content-width';
			}
			$wrapper_args = array(
			...
			if ( $this->has_overlay( $attributes ) ) {
				$overlay_type = ( ! empty( $attributes['currentOverlayTab'] ) ? $attributes['currentOverlayTab'] : 'normal' );
				$extra_content .= '<div class="bst-row-layout-overlay bst-row-overlay-' . esc_attr( $overlay_type ) . '' . ( 'normal' === $overlay_type && ! empty( $attributes['overlayBgImg'] ) && ! empty( $attributes['overlayBgImgAttachment'] ) && 'parallax' === $attributes['overlayBgImgAttachment'] ? ' bst-jarallax' : '' ) . '"></div>';
			}
			...
			if ( ! empty( $attributes['backgroundSettingTab'] ) && 'video' === $attributes['backgroundSettingTab'] && ! empty( $attributes['backgroundVideo'][0]['local'] ) && ( ( ! empty( $attributes['tabletBackground'][0]['enable'] ) && true == $attributes['tabletBackground'][0]['enable'] ) || ( ! empty( $attributes['mobileBackground'][0]['enable'] ) && true == $attributes['mobileBackground'][0]['enable'] ) ) && apply_filters( 'base_blocks_rowlayout_prevent_preload_for_mobile', true ) ) {
				if ( ! empty( $attributes['tabletBackground'][0]['enable'] ) && 'true' == $attributes['tabletBackground'][0]['enable'] ) {
					$size = 1024;
				} else {
					$size = 767;
				...
				if ( ! empty( $attributes['bgImg'] ) ) {
					$content = str_replace( 'bst-layout-id' . $attributes['uniqueID'] . '"><div class="bsb-blocks-bg-video-container"><video class="bsb-blocks-bg-video" poster="' . $attributes['bgImg'] . '" playsinline autoplay', 'bst-layout-id' . $attributes['uniqueID'] . '"><div class="bsb-blocks-bg-video-container"><video id="bg-row-video-' . $attributes['uniqueID'] . '" class="bsb-blocks-bg-video" poster=" ' . $attributes['bgImg'] . '" playsinline preload="none"', $content );
				} else {
					$content = str_replace( 'bst-layout-id' . $attributes['uniqueID'] . '"><div class="bsb-blocks-bg-video-container"><video class="bsb-blocks-bg-video" playsinline autoplay', 'bst-layout-id' . $attributes['uniqueID'] . '"><div class="bsb-blocks-bg-video-container"><video id="bg-row-video-' . $attributes['uniqueID'] . '" class="bsb-blocks-bg-video" playsinline preload="none"', $content );
				}
				$content = $content . '<script>if( window.innerWidth > ' . $size . ' ){document.getElementById("bg-row-video-' . $attributes['uniqueID'] . '").removeAttribute("preload");document.getElementById("bg-row-video-' . $attributes['uniqueID'] . '").setAttribute("autoplay","");}</script>';
			}
		}
	...
	public function register_scripts() {
		...
		wp_register_script( 'jarallax', BASE_BLOCKS_URL . 'includes/assets/js/jarallax.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-parallax-js', BASE_BLOCKS_URL . 'includes/assets/js/bst-init-parallax.min.js', array( 'jarallax' ), GUTENAM_BLOCKS_VERSION, true );
		wp_localize_script(
			'base-blocks-parallax-js',
			'base_blocks_parallax',
			array(
				'speed' => apply_filters( 'base_blocks_parallax_speed', -0.1 ),
			)
		);
		...
		wp_register_script( 'base-blocks-video-bg', BASE_BLOCKS_URL . 'includes/assets/js/bsb-init-html-bg-video.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
	}
}

Base_Blocks_Rowlayout_Block::get_instance();
