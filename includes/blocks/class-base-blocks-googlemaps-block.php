 ...
 * @category class
 */
class Base_Blocks_Googlemaps_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.bsb-google-maps-container' . $unique_id );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$style_id = 'bsb-google-maps' . esc_attr( $unique_id );
		if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'base_blocks_render_inline_css', true, 'google_maps', $attributes ) ) {
			// If filter didn't run in header (which would have enqueued the specific css id ) then filter attributes for easier dynamic css.
			$attributes = apply_filters( 'base_blocks_google_maps_render_block_attributes', $attributes );
		}

		if ( isset( $attributes['apiType'] ) && $attributes['apiType'] === 'javascript' ) {
			$this->enqueue_script( 'base-blocks-googlemaps-init-js' );
			$this->enqueue_script( 'base-blocks-googlemaps-js' );
		}

		if ( $updated_version ) {
			$wrapper_args = array(
				'class' => 'bsb-google-maps-container bsb-google-maps-container' . $unique_id . ' ' . ( ! empty( $attributes['align'] ) ? 'align' . $attributes['align'] : '' ),
			...
			if ( isset( $attributes['apiType'] ) && $attributes['apiType'] === 'javascript' ) {
				$content .= '<div id="bsb-google-map' . $unique_id . '" style="width: 100%; height: 100%"></div>';
			} else {
				...
				$content .= '<iframe width="100%" height="100%"
							...
							title="' . esc_attr( sprintf( __( 'Google map of %s', 'gutenam-blocks' ), $attributes['location'] ) ) . '"></iframe>';
			}
			$content .= '</div>';
		}
		...
		$user_google_maps_key = get_option( 'base_blocks_google_maps_api', '' );
		if ( empty( $user_google_maps_key ) ) {
			$content = str_replace( 'BASE_GOOGLE_MAPS_KEY', 'AIzaSyBAM2o7PiQqwk15LC1XRH2e_KJ-jUa7KYk', $content );
		} else {
			$content = str_replace( 'BASE_GOOGLE_MAPS_KEY', $user_google_maps_key, $content );
		}
		...
		$content .= '<script>';
		$content .= 'function bsb_google_map' . str_replace( '-', '_', $unique_id ) . '() {';
		$content .= ' let center = { lat: ' . $gMapLat . ', lng: ' . $gMapLng . '};';

		$content .= ' let map = new google.maps.Map(document.getElementById("bsb-google-map' . $unique_id . '"), {
					    zoom: ' . $zoom . ',
					    center: center,';
	...
	public function register_scripts() {
		...
		$google_maps_api_key = get_option( 'base_blocks_google_maps_api', 'missingkey' );
		wp_register_script( 'base-blocks-googlemaps-js', 'https://maps.googleapis.com/maps/api/js?key=' . $google_maps_api_key . '&callback=bbInitMaps', array( 'base-blocks-googlemaps-init-js' ), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-googlemaps-init-js', BASE_BLOCKS_URL . 'includes/assets/js/bsb-init-google-maps.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
	}


}

Base_Blocks_Googlemaps_Block::get_instance();
