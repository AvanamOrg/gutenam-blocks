 ...
 * @category class
 */
class Base_Blocks_Countdown_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		if ( isset( $attributes['background'] ) || isset( $attributes['border'] ) || ( isset( $attributes['borderRadius'] ) && is_array( $attributes['borderRadius'] ) ) || ( isset( $attributes['borderWidth'] ) && is_array( $attributes['borderWidth'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container-' . $unique_id );
		...
		if ( isset( $attributes['itemBackground'] ) || isset( $attributes['itemBorder'] ) || ( isset( $attributes['itemBorderRadius'] ) && is_array( $attributes['itemBorderRadius'] ) ) || ( isset( $attributes['itemBorderWidth'] ) && is_array( $attributes['itemBorderWidth'] ) ) || ( isset( $attributes['itemPadding'] ) && is_array( $attributes['itemPadding'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-date-item:not( .bsb-countdown-divider-item )' );
			...
			$css->set_selector( '.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-date-item.bsb-countdown-divider-item' );
			if ( isset( $attributes['itemPadding'] ) && is_array( $attributes['itemPadding'] ) ) {
				if ( isset( $attributes['itemPadding'][0] ) && is_numeric( $attributes['itemPadding'][0] ) ) {
					$css->add_property( 'padding-top', $attributes['itemPadding'][0] . ( isset( $attributes['itemPaddingType'] ) ? $attributes['itemPaddingType'] : 'px' ) );
				}
		...
		if ( ! empty( $number_font ) || ( isset( $attributes['numberColor'] ) && ! empty( $attributes['numberColor'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-date-item .bsb-countdown-number' );
			...
			if ( isset( $number_font['size'] ) && is_array( $number_font['size'] ) && isset( $number_font['size'][0] ) && ! empty( $number_font['size'][0] ) ) {
				$css->set_selector( '.bsb-countdown-container.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-date-item' );
				if( $css->is_variable_font_size_value( $number_font['size'][0] ) ) {
					$css->add_property( 'font-size', $css->get_variable_font_size_value( $number_font['size'][0] ) );
				} else {
					$css->add_property( 'font-size', $number_font['size'][0] . ( ! isset( $number_font['sizeType'] ) ? 'px' : $number_font['sizeType'] ) );
		...
		if ( ! empty( $label_font ) || ( isset( $attributes['labelColor'] ) && ! empty( $attributes['labelColor'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-date-item .bsb-countdown-label' );
		...
		if ( ! empty( $pre_label_font ) || ( isset( $attributes['preLabelColor'] ) && ! empty( $attributes['preLabelColor'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-item.bsb-pre-timer' );
		...
		if ( ! empty( $post_label_font ) || ( isset( $attributes['postLabelColor'] ) && ! empty( $attributes['postLabelColor'] ) ) ) {
			$css->set_selector( '.bsb-countdown-container.bsb-countdown-container-' . $unique_id . ' .bsb-countdown-item.bsb-post-timer' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$site_slug               = apply_filters( 'base_blocks_countdown_site_slug', sanitize_title( get_bloginfo( 'name' ) ) );
		...
		$countdown[ $unique_id ] = array(
			...
			'evergreen'    => ( 'evergreen' === $countdown_type ? apply_filters( 'base_blocks_countdown_evergreen_config', 'query', $campaign_id, $site_slug, $reset_days ) : '' ),
			...
			'daysLabel'    => ( isset( $attributes['daysLabel'] ) && ! empty( $attributes['daysLabel'] ) ? $attributes['daysLabel'] : esc_attr__( 'Days', 'gutenam-blocks' ) ),
			'hoursLabel'   => ( isset( $attributes['hoursLabel'] ) && ! empty( $attributes['hoursLabel'] ) ? $attributes['hoursLabel'] : esc_attr__( 'Hrs', 'gutenam-blocks' ) ),
			'minutesLabel' => ( isset( $attributes['minutesLabel'] ) && ! empty( $attributes['minutesLabel'] ) ? $attributes['minutesLabel'] : esc_attr__( 'Mins', 'gutenam-blocks' ) ),
			'secondsLabel' => ( isset( $attributes['secondsLabel'] ) && ! empty( $attributes['secondsLabel'] ) ? $attributes['secondsLabel'] : esc_attr__( 'Secs', 'gutenam-blocks' ) ),
		);

		wp_localize_script(
			'base-blocks-countdown',
			'base_blocks_countdown',
			array(
				'ajax_url'   => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( 'base_blocks_countdown' ),
				'site_slug'  => apply_filters( 'base_blocks_countdown_site_slug', sanitize_title( get_bloginfo( 'name' ) ) ),
	...
	public function register_scripts() {
		...
		wp_register_script( 'base-blocks-countdown', BASE_BLOCKS_URL . 'includes/assets/js/bsb-countdown.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
	}


}

Base_Blocks_Countdown_Block::get_instance();
