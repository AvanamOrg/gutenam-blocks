 ...
 * @category class
 */
class Base_Blocks_Form_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$this->enqueue_script( 'base-blocks-form' );

		if ( isset( $attributes['recaptcha'] ) && $attributes['recaptcha'] ) {
			if ( isset( $attributes['recaptchaVersion'] ) && 'v2' === $attributes['recaptchaVersion'] ) {
				$this->enqueue_script( 'base-blocks-google-recaptcha-v2' );
			} else {
				$this->enqueue_script( 'base-blocks-google-recaptcha-v3' );
			}
		}
		...
		$css->set_selector( '.wp-block-base-form.base-form-' . $unique_id . '.bsb-form-wrap' );
		...
		if ( isset( $attributes['style'] ) && is_array( $attributes['style'] ) && isset( $attributes['style'][0] ) && is_array( $attributes['style'][0] ) ) {
			...
			if ( isset( $style['rowGap'] ) && is_numeric( $style['rowGap'] ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field' );
				$css->add_property( 'margin-bottom', $style['rowGap'] . ( isset( $style['rowGapType'] ) && ! empty( $style['rowGapType'] ) ? $style['rowGapType'] : 'px' ) );

				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field.bsb-submit-field' );
			...
			if ( isset( $style['gutter'] ) && is_numeric( $style['gutter'] ) ) {
				...
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form' );
			...
			if ( isset( $style['requiredColor'] ) && ! empty( $style['requiredColor'] ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field label .required' );
			...
			if ( ( isset( $style['color'] ) && ! empty( $style['color'] ) ) || ( isset( $style['background'] ) && ! empty( $style['background'] ) ) || ( isset( $style['border'] ) && ! empty( $style['border'] ) ) || ( isset( $style['backgroundType'] ) && 'gradient' === $style['backgroundType'] ) || ( isset( $style['boxShadow'] ) && is_array( $style['boxShadow'] ) && isset( $style['boxShadow'][0] ) && true === $style['boxShadow'][0] ) || ( isset( $style['borderRadius'] ) && is_numeric( $style['borderRadius'] ) ) || ( isset( $style['fontSize'] ) && is_array( $style['fontSize'] ) ) || ( isset( $style['lineHeight'] ) && is_array( $style['lineHeight'] ) && is_numeric( $style['lineHeight'][0] ) ) || ( isset( $style['borderWidth'] ) && is_array( $style['borderWidth'] ) && is_numeric( $style['borderWidth'][0] ) ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-text-style-field, .base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-select-style-field' );
			...
			if ( ( isset( $style['colorActive'] ) && ! empty( $style['colorActive'] ) ) || ( isset( $style['backgroundActive'] ) && ! empty( $style['backgroundActive'] ) ) || ( isset( $style['borderActive'] ) && ! empty( $style['borderActive'] ) ) || ( isset( $style['backgroundActiveType'] ) && 'gradient' === $style['backgroundActiveType'] ) || ( isset( $style['boxShadow'] ) && is_array( $style['boxShadow'] ) && isset( $style['boxShadow'][0] ) && true === $style['boxShadow'][0] ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-text-style-field:focus, .base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-select-style-field:focus' );
			...
			if ( isset( $style['size'] ) && 'custom' && $style['size'] && isset( $style['deskPadding'] ) && is_array( $style['deskPadding'] ) && isset( $style['deskPadding'][0] ) && is_numeric( $style['deskPadding'][0] ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-text-style-field' );
		...
		if ( isset( $attributes['labelFont'] ) && is_array( $attributes['labelFont'] ) && isset( $attributes['labelFont'][0] ) && is_array( $attributes['labelFont'][0] ) ) {
			$label_font = $attributes['labelFont'][0];
			$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field > label' );
			...
			if ( !empty( $label_font['lineHeight'][0] ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field.bsb-accept-form-field .bsb-checkbox-style' );
		...
		if ( isset( $attributes['submit'] ) && is_array( $attributes['submit'] ) && isset( $attributes['submit'][0] ) && is_array( $attributes['submit'][0] ) ) {
			...
			$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-forms-submit' );
			...
			$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-forms-submit:hover, .base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-forms-submit:focus ' );
			...
			if ( 'gradient' === $bgtype ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .bsb-form .base-blocks-form-field .bsb-forms-submit::before' );
		...
		if ( isset( $attributes['messageFont'] ) && is_array( $attributes['messageFont'] ) && isset( $attributes['messageFont'][0] ) && is_array( $attributes['messageFont'][0] ) ) {
			...
			if ( ( isset( $message_font['colorSuccess'] ) && ! empty( $message_font['colorSuccess'] ) ) || ( isset( $message_font['backgroundSuccess'] ) && ! empty( $message_font['backgroundSuccess'] ) ) || ( isset( $message_font['backgroundSuccess'] ) && ! empty( $message_font['backgroundSuccess'] ) ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .base-blocks-form-success' );
			...
			if ( ( isset( $message_font['colorError'] ) && ! empty( $message_font['colorError'] ) ) || ( isset( $message_font['backgroundError'] ) && ! empty( $message_font['backgroundError'] ) ) || ( isset( $message_font['backgroundError'] ) && ! empty( $message_font['backgroundError'] ) ) ) {
				$css->set_selector( '.base-form-' . $unique_id . ' .base-blocks-form-warning' );
			...
			$css->set_selector( '.base-form-' . $unique_id . ' .base-blocks-form-message, .base-form-' . $unique_id . ' .bsb-form-error-msg' );
			...
			if ( ( isset( $message_font['size'] ) && is_array( $message_font['size'] ) && is_numeric( $message_font['size'][1] ) ) || ( isset( $message_font['lineHeight'] ) && is_array( $message_font['lineHeight'] ) && is_numeric( $message_font['lineHeight'][1] ) ) ) {
				$css->set_media_state( 'tablet' );
				$css->set_selector( '.base-form-' . $unique_id . ' .base-blocks-form-message' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		$content .= '<noscript><div class="base-blocks-form-message base-blocks-form-warning">' . __( 'Please enable JavaScript in your browser to submit the form', 'gutenam-blocks' ) . '</div><style>.base-form-' . $unique_id . ' .base-blocks-form-field.bsb-submit-field { display: none; }</style></noscript>';

		// Update honeypot autocomplete value.
		if( ( !isset( $attributes['honeyPot']) || isset( $attributes['honeyPot'] ) && $attributes['honeyPot'] ) ) {
			$default = '<input class="base-blocks-field verify" type="text" name="_bsb_verify_email" autocomplete="off" aria-hidden="true" placeholder="Email" tabindex="-1"/>';
			$new     = '<input class="base-blocks-field verify" type="text" name="_bsb_verify_email" autocomplete="new-password" aria-hidden="true" placeholder="Email" tabindex="-1" data-1p-ignore="true" data-lpignore="true" />';
	...
	public function register_scripts() {
		...
		wp_register_script( 'base-blocks-form', BASE_BLOCKS_URL . 'includes/assets/js/bsb-form-block.min.js', array(), GUTENAM_BLOCKS_VERSION, true );

		$recaptcha_site_key = get_option( 'base_blocks_recaptcha_site_key' );
		if ( ! $recaptcha_site_key ) {
			$recaptcha_site_key = 'missingkey';
		}

		$recaptcha_lang = !empty( get_option('base_blocks_recaptcha_language') ) ? '&hl=' . get_option('base_blocks_recaptcha_language') : '';

		wp_register_script( 'base-blocks-google-recaptcha-v3', 'https://www.google.com/recaptcha/api.js?render=' . esc_attr( $recaptcha_site_key ) . $recaptcha_lang, array(), GUTENAM_BLOCKS_VERSION, true );
		$recaptcha_script = "grecaptcha.ready(function () { var recaptchaResponse = document.getElementById('bsb_recaptcha_response'); if ( recaptchaResponse ) { grecaptcha.execute('" . esc_attr( $recaptcha_site_key ) . "', { action: 'bsb_form' }).then(function (token) { recaptchaResponse.value = token; }); } var bsb_recaptcha_inputs = document.getElementsByClassName('bsb_recaptcha_response'); if ( ! bsb_recaptcha_inputs.length ) { return; } for (var i = 0; i < bsb_recaptcha_inputs.length; i++) { const e = i; grecaptcha.execute('" . esc_attr( $recaptcha_site_key ) . "', { action: 'bsb_form' }).then(function (token) { bsb_recaptcha_inputs[e].setAttribute('value', token); }); } });";
		wp_add_inline_script( 'base-blocks-google-recaptcha-v3', $recaptcha_script, 'after' );
		wp_register_script( 'base-blocks-google-recaptcha-v2', 'https://www.google.com/recaptcha/api.js?render=explicit&onload=bbOnloadV2Callback' . $recaptcha_lang, array( 'jquery' ), GUTENAM_BLOCKS_VERSION, true );
		$recaptcha_v2_script = "var bbOnloadV2Callback = function(){jQuery( '.wp-block-base-form' ).find( '.base-blocks-g-recaptcha-v2' ).each( function() {grecaptcha.render( jQuery( this ).attr( 'id' ), {'sitekey' : '" . esc_attr( $recaptcha_site_key ) . "'});});}";
		wp_add_inline_script( 'base-blocks-google-recaptcha-v2', $recaptcha_v2_script, 'before' );

		wp_localize_script(
			'base-blocks-form',
			'base_blocks_form_params',
			array(
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'error_message' => __( 'Please fix the errors to proceed', 'gutenam-blocks' ),
				'nonce'         => wp_create_nonce( 'bsb_form_nonce' ),
				'required'      => __( 'is required', 'gutenam-blocks' ),
				'mismatch'      => __( 'does not match', 'gutenam-blocks' ),
				'validation'    => __( 'is not valid', 'gutenam-blocks' ),
				'duplicate'     => __( 'requires a unique entry and this value has already been used', 'gutenam-blocks' ),
				'item'          => __( 'Item', 'gutenam-blocks' ),
			)
		);
...
Base_Blocks_Form_Block::get_instance();
