 ...
 * @category class
 */
class Base_Blocks_Image_Block extends Base_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.wp-block-base-image.bsb-image' . $unique_id . ':not(.bsb-specificity-added):not(.bsb-extra-specificity-added)' );
		...
		$css->set_selector( '.wp-block-base-image.bsb-image' . $unique_id );
		...
		if ( $align !== 'wide' && $align !== 'full' ) {
			$css->set_selector( '.bsb-image' . $unique_id . '.bsb-image-is-ratio-size, .bsb-image' . $unique_id . ' .bsb-image-is-ratio-size' );
		...
		if ( $align === 'center' || $align === 'right' || $align === 'left' ) {
			$css->set_selector( '.bsb-image' . $unique_id . ' figure' );
			if ( isset( $attributes['imgMaxWidth'] ) && is_numeric( $attributes['imgMaxWidth'] ) ) {
				$css->add_property( 'max-width', $attributes['imgMaxWidth'] . 'px' );
				$css->set_selector( '.bsb-image' . $unique_id . ' .image-is-svg, .bsb-image' . $unique_id . ' .image-is-svg img' );
				$css->add_property( 'width', '100%' );
			}
		} else if ( $align !== 'wide' && $align !== 'full' ) {
			$css->set_selector( '.bsb-image' . $unique_id );
			if ( isset( $attributes['imgMaxWidth'] ) && is_numeric( $attributes['imgMaxWidth'] ) ) {
				$css->add_property( 'max-width', $attributes['imgMaxWidth'] . 'px' );
				$css->set_selector( '.image-is-svg.bsb-image' . $unique_id );
				$css->add_property( 'flex', '0 1 100%' );
				$css->set_selector( '.image-is-svg.bsb-image' . $unique_id . ' img' );
				$css->add_property( 'width', '100%' );
			}
		}
		...
		$css->set_selector( '.bsb-image' . $unique_id . ':not(.bsb-image-is-ratio-size) .bsb-img, .bsb-image' . $unique_id . '.bsb-image-is-ratio-size' );
		...
		$css->set_selector( '.bsb-image' . $unique_id . ' img.bsb-img, .bsb-image' . $unique_id . ' .bsb-img img' );
		...
		if ( ! empty( $attributes['maskSvg'] ) && 'none' !== $attributes['maskSvg'] ) {
			if ( 'custom' === $attributes['maskSvg'] ) {
				...
				$mask_base_url = BASE_BLOCKS_URL . 'includes/assets/images/masks/';
		...
		if ( ! isset( $attributes['showCaption'] ) && isset( $attributes['captionStyles'] ) && is_array( $attributes['captionStyles'] ) && is_array( $attributes['captionStyles'][0] ) ) {
			...
			$css->set_selector( '.bsb-image' . $unique_id . ' figcaption' );
...
Base_Blocks_Image_Block::get_instance();
