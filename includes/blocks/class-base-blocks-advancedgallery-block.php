 ...
 * @category class
 */
class Base_Blocks_Advancedgallery_Block extends Base_Blocks_Abstract_Block {
	...
	public function render_scripts( $attributes, $inline = false ) {
		if ( $this->has_style ) {
			if ( ! wp_style_is( 'base-blocks-' . $this->block_name, 'enqueued' ) ) {
				$this->enqueue_style( 'base-blocks-' . $this->block_name );
				if ( $inline ) {
					$this->should_render_inline_stylesheet( 'base-blocks-' . $this->block_name );
				}
			}
		}
		if ( isset( $attributes['type'] ) && ( 'carousel' === $attributes['type'] || 'fluidcarousel' === $attributes['type'] || 'slider' === $attributes['type'] || 'thumbslider' === $attributes['type'] ) ) {
			$this->enqueue_style( 'base-blocks-splide' );
			$this->should_render_inline_stylesheet( 'base-blocks-splide' );

			$this->enqueue_script( 'base-blocks-splide-init' );

		} elseif ( ! isset( $attributes['type'] ) || ( isset( $attributes['type'] ) && 'masonry' === $attributes['type'] ) ) {
			$this->enqueue_script( 'base-blocks-masonry-init' );
		}
		if ( isset( $attributes['type'] ) && ( 'tiles' === $attributes['type'] || 'thumbslider' === $attributes['type'] ) ) {
			if ( wp_style_is( 'base-blocks-gallery-pro', 'registered' ) ) {
				$this->enqueue_style( 'base-blocks-gallery-pro' );
				$this->should_render_inline_stylesheet( 'base-blocks-gallery-pro' );
			}
		}
		if ( isset( $attributes['linkTo'] ) && 'media' == isset( $attributes['linkTo'] ) && isset( $attributes['lightbox'] ) && 'magnific' === $attributes['lightbox'] ) {
			...
			$this->should_render_inline_stylesheet( 'base-glightbox' );
			$this->enqueue_script( 'base-blocks-glight-init' );
		}
	}
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		$updated_version = ! empty( $attributes['bbVersion'] ) && $attributes['bbVersion'] > 1 ? true : false;
		$css->set_style_id( 'bsb-' . $this->block_name . $unique_style_id );
		$gallery_type = ! empty( $attributes['type'] ) ? $attributes['type'] : 'masonry';

		$css->set_selector('.wp-block-base-advancedgallery.bsb-gallery-wrap-id-' . $unique_id );
		...
		if ( 'masonry' === $gallery_type ) {
			$css->set_selector('.wp-block-base-advancedgallery .bsb-gallery-type-masonry.bsb-gallery-id-' . $unique_id );
			$css->add_property('margin','-' . ( $desk_gutter / 2 ) . $gutter_unit );
			$css->set_selector('.bsb-gallery-type-masonry.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item' );
			$css->add_property('padding', ( $desk_gutter / 2 ) . $gutter_unit );
		} else if ( 'grid' === $gallery_type ) {
			// Grid.
			$css->set_selector('.wp-block-base-advancedgallery .bsb-gallery-type-grid.bsb-gallery-id-' . $unique_id );
			$css->add_property('margin','-' . ( $desk_gutter / 2 ) . $gutter_unit );
			$css->set_selector('.bsb-gallery-type-grid.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item' );
			$css->add_property('padding', ( $desk_gutter / 2 ) . $gutter_unit );
		} else if ( 'tiles' === $gallery_type ) {
			// Tiles.
			$css->set_selector('.wp-block-base-advancedgallery .bsb-gallery-type-tiles.bsb-gallery-id-' . $unique_id );
			$css->add_property('margin','-' . ( $desk_gutter / 2 ) . $gutter_unit );
			$css->set_selector('.bsb-gallery-type-tiles.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item' );
			$css->add_property('padding', ( $desk_gutter / 2 ) . $gutter_unit );
		} else if ( 'carousel' === $gallery_type ) {
			// Carousel.
			if ( ! $updated_version ) {
				$css->set_selector('.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init' );
				$css->add_property('margin', '0 -' . ( $desk_gutter / 2 ) . $gutter_unit );

				$css->set_selector('.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init .bsb-slide-item');
				$css->add_property('padding', '4px ' . ( $desk_gutter / 2 ) . $gutter_unit );
				$css->set_selector('.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init .slick-prev' );
				$css->add_property('left', ( $desk_gutter / 2 ) . $gutter_unit );

				$css->set_selector('.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init .slick-next' );
				$css->add_property('right', ( $desk_gutter / 2 ) . $gutter_unit );
			} else {
				$css->set_selector( '.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init:not( .splide-initialized )' );
				$css->add_property( 'margin-left', '-' . ( $desk_gutter / 2 ) . $gutter_unit );
				$css->add_property( 'margin-right', '-' . ( $desk_gutter / 2 ) . $gutter_unit );

				$css->set_selector( '.bsb-gallery-type-carousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init:not( .splide-initialized ) .bsb-slide-item' );
			...
			if ( ! $updated_version ) {
				// Fluid Carousel.
				$css->set_selector('.bsb-gallery-type-fluidcarousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init .bsb-slide-item');
				$css->add_property('padding', '4px ' . ( $desk_gutter / 2 ) . $gutter_unit );
				$css->set_selector('.bsb-gallery-type-fluidcarousel.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel .bst-blocks-carousel-init.bsb-carousel-mode-align-left .bsb-slide-item' );
				$css->add_property('padding', '4px ' . ( $desk_gutter ) . $gutter_unit . ' 4px 0' );
			}
		} else if ( 'thumbslider' === $gallery_type ) {
			if ( $updated_version ) {
				// Thumbnail.
				$css->set_selector('.bsb-gallery-type-thumbslider.bsb-gallery-id-' . $unique_id . ' .bst-blocks-carousel-init ');
		...
		if ( isset( $attributes['type'] ) && 'fluidcarousel' === $attributes['type'] && isset( $attributes['carouselHeight'] ) && is_array( $attributes['carouselHeight'] ) ) {
			if ( isset( $attributes['carouselHeight'][0] ) && is_numeric( $attributes['carouselHeight'][0] ) ) {
				$css->set_selector('.bsb-gallery-id-' . $unique_id . '.bsb-gallery-ul.bsb-gallery-type-fluidcarousel .bst-blocks-carousel figure .bsb-gal-image-radius, .bsb-gallery-id-' . $unique_id . '.bsb-gallery-ul.bsb-gallery-type-fluidcarousel .bst-blocks-carousel figure .bsb-gal-image-radius img' );
		...
		if ( isset( $attributes['type'] ) && 'tiles' === $attributes['type'] && isset( $attributes['carouselHeight'] ) && is_array( $attributes['carouselHeight'] ) ) {
			if ( isset( $attributes['carouselHeight'][0] ) && is_numeric( $attributes['carouselHeight'][0] ) ) {
				$css->set_selector('.bsb-gallery-id-' . $unique_id . '.bsb-gallery-ul.bsb-gallery-type-tiles .base-blocks-gallery-item .base-blocks-gallery-item-inner img, .bsb-gallery-id-' . $unique_id . '.bsb-gallery-ul.bsb-gallery-type-tiles > .base-blocks-gallery-item' );
				$css->add_property('height', $attributes['carouselHeight'][0] . 'px' );
			}
			if ( isset( $attributes['carouselHeight'][1] ) && is_numeric( $attributes['carouselHeight'][1] ) ) {
				$css->set_media_state( 'tablet' );
		...
		if ( isset( $attributes['imageRadius'] ) && is_array( $attributes['imageRadius'] ) && isset( $attributes['imageRadius'][0] ) && is_numeric( $attributes['imageRadius'][0] ) ) {
			$css->set_selector('.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item .bsb-gal-image-radius' );
		...
		if ( isset( $attributes['displayShadow'] ) && ! empty( $attributes['displayShadow'] ) && true === $attributes['displayShadow'] ) {
			...
			if ( isset( $attributes['shadowHover'] ) && is_array( $attributes['shadowHover'] ) && is_array( $attributes['shadowHover'][ 0 ] ) ) {
				...
				$css->set_selector('.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item:hover .bsb-gal-image-radius' );
		...
		if ( isset( $attributes['showCaption'] ) && true === $attributes['showCaption'] && isset( $attributes['captionStyles'] ) && is_array( $attributes['captionStyles'] ) && is_array( $attributes['captionStyles'][0] ) ) {
			...
			$css->set_selector('.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item .base-blocks-gallery-item-inner .base-blocks-gallery-item__caption' );
			...
			if ( isset( $caption_font['background'] ) && ! empty( $caption_font['background'] ) ) {
				$css->set_selector('.bsb-gallery-caption-style-cover-hover.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item .base-blocks-gallery-item-inner .base-blocks-gallery-item__caption, .bsb-gallery-caption-style-below.bsb-gallery-id-' . $unique_id . ' .base-blocks-gallery-item .base-blocks-gallery-item-inner .base-blocks-gallery-item__caption' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		if ( ! empty( $attributes['bbVersion'] ) && $attributes['bbVersion'] > 1 ) {
			...
			$outer_classes = array( 'bsb-gallery-wrap-id-' . $unique_id );
			$outer_classes[] = ! empty( $attributes['align'] ) ? 'align' . $attributes['align'] : 'alignnone';
			// Gallery Class.
			$gallery_classes = array( 'bsb-gallery-ul', 'bsb-gallery-non-static' );
			$gallery_classes[] = 'bsb-gallery-type-' . esc_attr( $type );
			if ( 'masonry' === $type ) {
				$gallery_classes[] = 'bsb-masonry-init';
			}
			if ( isset( $attributes['mobileForceHover'] ) && true === $attributes['mobileForceHover'] ) {
				$gallery_classes[] = 'bsb-mobile-force-hover';
			}
			$gallery_classes[] = 'bsb-gallery-id-' . esc_attr( $unique_id );
			$gallery_classes[] = 'bsb-gallery-caption-style-' . ( ! empty( $attributes['captionStyle'] ) ? esc_attr( $attributes['captionStyle'] ) : 'bottom-hover' );
			$gallery_classes[] = 'bsb-gallery-filter-' . ( ! empty( $attributes['imageFilter'] ) ? esc_attr( $attributes['imageFilter'] ) : 'none' );
			if ( 'media' === $link_to && 'magnific' === $lightbox ) {
				$gallery_classes[] = 'bsb-gallery-magnific-init';
			}
			$wrapper_args = array(
			...
			switch ( $type ) {
				case 'carousel':
					...
					$content .= '<div class="bst-blocks-carousel bst-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '">';
					$content .= '<div class="bst-blocks-carousel-init bsb-gallery-carousel bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . '" data-columns-xxl="' . esc_attr( $columns_xxl ) . '" data-columns-xl="' . esc_attr( $columns_xl ) . '" data-columns-md="' . esc_attr( $columns_md ) . '" data-columns-sm="' . esc_attr( $columns_sm ) . '" data-columns-xs="' . esc_attr( $columns_xs ) . '" data-columns-ss="' . esc_attr( $columns_ss ) . '" data-slider-anim-speed="' . esc_attr( $trans_speed ) . '" data-slider-scroll="' . esc_attr( $slides_sc ) . '" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-dots="' . esc_attr( 'none' === $dot_style ? 'false' : 'true' ) . '" data-slider-hover-pause="false" data-slider-auto="' . esc_attr( $autoplay ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '" data-slider-gap="' . esc_attr( $gap . $gap_unit ) . '" data-slider-gap-tablet="' . esc_attr( $tablet_gap . $gap_unit ) . '" data-slider-gap-mobile="' . esc_attr( $mobile_gap . $gap_unit ) . '">';
					foreach ( $images as $key => $image ) {
						$content .= '<div class="bsb-slide-item bsb-gallery-carousel-item">';
				...
				case 'fluidcarousel':
					$content .= '<div class="' . esc_attr( implode( ' ', $gallery_classes ) ) . '" data-image-filter="' . esc_attr( $image_filter ) . '" data-lightbox-caption="' . ( $lightbox_cap ? 'true' : 'false' ) . '">';
					$content .= '<div class="bst-blocks-carousel bst-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '">';
					$content .= '<div class="bst-blocks-carousel-init bsb-blocks-fluid-carousel bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . ( $car_align ? '' : ' bsb-carousel-mode-align-left' ) . '" data-slider-anim-speed="' . esc_attr( $trans_speed ) . '" data-slider-scroll="1" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-dots="' . esc_attr( 'none' === $dot_style ? 'false' : 'true' ) . '" data-slider-hover-pause="false" data-slider-auto="' . esc_attr( $autoplay ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '" data-slider-type="fluidcarousel" data-slider-center-mode="' . esc_attr( ( $car_align ? 'true' : 'false' ) ) . '" data-slider-gap="' . esc_attr( $gap . $gap_unit ) . '" data-slider-gap-tablet="' . esc_attr( $tablet_gap . $gap_unit ) . '" data-slider-gap-mobile="' . esc_attr( $mobile_gap . $gap_unit ) . '">';
					foreach ( $images as $key => $image ) {
						$content .= '<div class="bsb-slide-item bsb-gallery-carousel-item">';
						$content .= $this->render_gallery_images( $image, $attributes );
						$content .= '</div>';
				...
				case 'slider':
					$content .= '<div class="' . esc_attr( implode( ' ', $gallery_classes ) ) . '" data-image-filter="' . esc_attr( $image_filter ) . '" data-lightbox-caption="' . ( $lightbox_cap ? 'true' : 'false' ) . '">';
					$content .= '<div class="bst-blocks-carousel bst-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '">';
					$content .= '<div class="bst-blocks-carousel-init bsb-blocks-slider bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . '" data-slider-anim-speed="' . esc_attr( $trans_speed ) . '" data-slider-scroll="1" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-fade="' . ( $slider_fade ? 'true' : 'false' ) . '" data-slider-dots="' . esc_attr( 'none' === $dot_style ? 'false' : 'true' ) . '"data-slider-type="slider"  data-slider-hover-pause="false" data-slider-auto="' . esc_attr( $autoplay ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '">';
					foreach ( $images as $key => $image ) {
						$content .= '<div class="bsb-slide-item bsb-gallery-slide-item">';
						$content .= $this->render_gallery_images( $image, $attributes );
						$content .= '</div>';
					}
					$content .= '</div>';
				...
				case 'thumbslider':
					$content .= '<div class="' . esc_attr( implode( ' ', $gallery_classes ) ) . '" data-image-filter="' . esc_attr( $image_filter ) . '" data-lightbox-caption="' . ( $lightbox_cap ? 'true' : 'false' ) . '">';
					$content .= '<div class="bst-blocks-carousel bst-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '">';
					$content .= '<div id="bsb-slider-' . esc_attr( $unique_id ) . '" class="bst-blocks-carousel-init bsb-blocks-slider bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . '" data-columns-xxl="' . esc_attr( $tcolumns_xxl ) . '" data-columns-xl="' . esc_attr( $tcolumns_xl ) . '" data-columns-md="' . esc_attr( $tcolumns_md ) . '" data-columns-sm="' . esc_attr( $tcolumns_sm ) . '" data-columns-xs="' . esc_attr( $tcolumns_xs ) . '" data-columns-ss="' . esc_attr( $tcolumns_ss ) . '" data-slider-anim-speed="' . esc_attr( $trans_speed ) . '" data-slider-scroll="1" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-fade="' . ( $slider_fade ? 'true' : 'false' ) . '" data-slider-dots="false" data-slider-hover-pause="false" data-slider-type="thumbnail" data-slider-nav="bsb-thumb-slider-' . esc_attr( $unique_id ) . '"  data-slider-auto="' . esc_attr( $autoplay ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '" data-slider-gap="' . esc_attr( $gap . $gap_unit ) . '" data-slider-gap-tablet="' . esc_attr( $tablet_gap . $gap_unit ) . '" data-slider-gap-mobile="' . esc_attr( $mobile_gap . $gap_unit ) . '">';
					foreach ( $images as $key => $image ) {
						$content .= '<div class="bsb-slide-item bsb-gallery-carousel-item">';
						$content .= $this->render_gallery_images( $image, $attributes );
						$content .= '</div>';
					...
					$content .= '</div>';
					$content .= '<div id="bsb-thumb-slider-' . esc_attr( $unique_id ) . '" class="bsb-blocks-slider bst-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' bst-carousel-dotstyle-' . esc_attr( $dot_style ) . '" data-slider-anim-speed="' . esc_attr( $trans_speed ) . '" data-slider-scroll="1" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-fade="' . ( $slider_fade ? 'true' : 'false' ) . '" data-slider-dots="false" data-slider-hover-pause="false" data-slider-auto="' . esc_attr( $autoplay ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '" data-slider-type="thumbnail" data-slider-nav="bsb-slider-' . esc_attr( $unique_id ) . '">';
					foreach ( $images as $key => $image ) {
						$content .= '<div class="bsb-slide-item bsb-gallery-carousel-item">';
	...
	public function render_gallery_images( $image, $attributes ) {
		...
		$link_target   = apply_filters( 'base_blocks_pro_dynamic_gallery_link_target', $link_target, $image, $attributes );
		$lightbox      = ( ! empty( $attributes['lightbox'] ) ? $attributes['lightbox'] : 'none' );
		$lightbox_cap = ( ! empty( $attributes['lightboxCaption'] ) && $attributes['lightboxCaption'] ? true : false );
		...
		$caption       = apply_filters( 'base_blocks_pro_dynamic_gallery_caption', $caption, $image, $attributes );
		$href = '';
		...
		switch ( $link_to ) {
			case 'media':
				...
				break;
			case 'custom':
				if ( ! empty( $image_id ) && isset( $attributes['baseDynamic'] ) && is_array( $attributes['baseDynamic'] ) && isset( $attributes['baseDynamic']['link'] ) && is_array( $attributes['baseDynamic']['link'] ) && isset( $attributes['baseDynamic']['link']['enable'] ) && $attributes['baseDynamic']['link']['enable'] ) {
					if ( ! empty( $attributes['baseDynamic']['link']['field'] ) && strpos( $attributes['baseDynamic']['link']['field'], '|' ) !== false ) {
						$field_split = explode( '|', $attributes['baseDynamic']['link']['field'], 2 );
					...
					$args = array(
						...
						'type'         => 'link',
						'field'        => $field,
						'custom'       => $attributes['baseDynamic']['link']['custom'],
						'para'         => $attributes['baseDynamic']['link']['para'],
						'force-string' => false,
						'before'       => $attributes['baseDynamic']['link']['before'],
						'after'        => null,
						'fallback'     => false,
						'relate'       => ( isset( $blockattr['baseDynamic']['link']['relate'] ) ? $blockattr['baseDynamic']['link']['relate'] : '' ),
						'relcustom'    => ( isset( $blockattr['baseDynamic']['link']['relcustom'] ) ? $blockattr['baseDynamic']['link']['relcustom'] : '' ),
					);
		...
		$image_contain_classes = array( 'bsb-gallery-image-contain' );
		if ( ( ( 'grid' === $type || 'carousel' === $type || 'slider' === $type || 'thumbslider' === $type ) && ! empty( $image_ratio ) ) || ( 'fluidcarousel' !== $type && 'tiles' !== $type && ! empty( $image['width'] ) && ! empty( $image['height'] ) ) ) {
			$image_contain_classes[] = 'base-blocks-gallery-intrinsic';
		}
		if ( ! empty( $image_ratio ) && ( 'grid' === $type || 'carousel' === $type || 'slider' === $type || 'thumbslider' === $type ) ) {
			$image_contain_classes[] = 'bsb-gallery-image-ratio-' . $image_ratio;
		}
		$fig_classes = array( 'bsb-gallery-figure' );
		if ( ! empty( $href ) ) {
			$fig_classes[] = 'bsb-gallery-item-has-link';
		}
		if ( $show_caption ) {
			if ( ! empty( $caption ) ) {
				$fig_classes[] = 'base-blocks-gallery-item-has-caption';
			}
		} else {
			$fig_classes[] = 'base-blocks-gallery-item-hide-caption';
		}
		if ( ! empty( $image_ratio ) && ( 'grid' === $type || 'carousel' === $type || 'slider' === $type || 'thumbslider' === $type ) ) {
			$image_contain_classes[] = 'bsb-has-image-ratio-' . $image_ratio;
		}
		...
		$figcap = '<' . $fig_tag . ' class="base-blocks-gallery-item__caption">' . ( ! empty( $caption ) && is_string( $caption ) ? $caption : '' ) . '</' . $fig_tag . '>';
		...
		$output = '<' . $item_tag . ' class="base-blocks-gallery-item">';
		$output .= '<div class="base-blocks-gallery-item-inner">';
		$output .= '<figure class="' . esc_attr( implode( ' ', $fig_classes ) ) . '">';
		if ( ! empty( $href ) ) {
			$output .= '<a href="' . esc_url( $href ) . '"' . ( $link_to === 'media' && $lightbox === 'magnific' && $lightbox_cap && ! empty( $caption ) && is_string( $caption ) ? ' data-description="' . esc_attr( $caption ) . '"' : '' ) . '' . ( $link_to === 'media' && $lightbox === 'magnific' && ! empty( $image_alt ) && is_string( $image_alt ) ? ' data-alt="' . esc_attr( $image_alt ) . '"' : '' ) . ' class="bsb-gallery-item-link" ' . ( ( $link_to === 'custom' && '_blank' === $link_target ) || ( $link_to === 'media' && $lightbox === 'new_tab' ) ? 'target="_blank"' : '' ) . ' ' . ( ( $link_to === 'custom' && ! empty( $rel_attr ) ) || ( $link_to === 'media' && ! empty( $rel_attr ) ) ? 'rel="' . esc_attr( $rel_attr ) . '"' : '' ) . '>';
		}
		$output .= '<div class="bsb-gal-image-radius" ' . ( ! empty( $padding_bottom ) ? 'style="max-width:' . $image['width'] . 'px;"' : '' ) . '>';
		$output .= $img;
	...
	public function render_gallery_thumb_images( $image, $attributes ) {
		...
		$image_full = ( ! empty( $image['fullUrl'] ) ? $image['fullUrl'] : $image['url'] );
		$image_contain_classes = array( 'bsb-gallery-image-contain base-blocks-gallery-intrinsic' );
		...
		$output = '<div class="base-blocks-gallery-thumb-item">';
		$output .= '<div class="base-blocks-gallery-thumb-item-inner">';
		$output .= '<figure class="' . esc_attr( implode( ' ', $fig_classes ) ) . '">';
		$output .= '<div class="bsb-gal-image-radius" ' . ( ! empty( $padding_bottom ) ? 'style="max-width:' . $image['width'] . 'px;"' : '' ) . '>';
		$output .= $img;
		$output .= '</div>';
	...
	public function register_scripts() {
		...
		wp_register_style( 'bst-splide', BASE_BLOCKS_URL . 'includes/assets/css/base-splide.min.css', array(), GUTENAM_BLOCKS_VERSION );
		wp_register_style( 'base-blocks-splide', BASE_BLOCKS_URL . 'includes/assets/css/bsb-blocks-splide.min.css', array( 'bst-splide' ), GUTENAM_BLOCKS_VERSION );
		wp_register_style( 'base-glightbox', BASE_BLOCKS_URL . 'includes/assets/css/bsb-glightbox.min.css', array(), GUTENAM_BLOCKS_VERSION );
		wp_register_script( 'bst-splide', BASE_BLOCKS_URL . 'includes/assets/js/splide.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-splide-init', BASE_BLOCKS_URL . 'includes/assets/js/bsb-splide-init.min.js', array( 'bst-splide' ), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-masonry-init', BASE_BLOCKS_URL . 'includes/assets/js/bsb-masonry-init.min.js', array( 'masonry' ), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-glightbox', BASE_BLOCKS_URL . 'includes/assets/js/glightbox.min.js', array(), GUTENAM_BLOCKS_VERSION, true );
		wp_register_script( 'base-blocks-glight-init', BASE_BLOCKS_URL . 'includes/assets/js/bsb-gallery-glight-init.min.js', array( 'base-glightbox' ), GUTENAM_BLOCKS_VERSION, true );
		wp_localize_script(
			'base-blocks-glight-init',
			'bsb_glightbox',
			array(
				'moreText' => __( 'See more', 'gutenam-blocks' ),
			)
		);
	}
}

Base_Blocks_Advancedgallery_Block::get_instance();
