 ...
 * @category class
 */
class Base_Blocks_Icon_Block extends Base_Blocks_Abstract_Block {
	/**
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		if ( ! empty( $attributes['icons'] ) && is_array( $attributes['icons'] ) ) {
			foreach ( $attributes['icons'] as $icon_key => $icon_value ) {
				if ( is_array( $icon_value ) ) {
					$css->set_selector( '.bst-svg-icons' . $unique_id . ' .bst-svg-item-' . $icon_key . ' .bsb-svg-icon-wrap' );
					...
					// Hover.
					$css->set_selector( '.bst-svg-icons' . $unique_id . ' .bst-svg-item-' . $icon_key . ':hover .bsb-svg-icon-wrap' );
		...
		$css->set_selector( '.wp-block-base-icon.bst-svg-icons' . $unique_id );
		$align_args = array(
...
Base_Blocks_Icon_Block::get_instance();
