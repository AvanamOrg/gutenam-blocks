...
class Base_Blocks_Frontend {
	...
	public function conditionally_render_block( $block_content, $block, $wp_block ) {
		...
		if ( 'base/rowlayout' === $block['blockName'] && isset( $block['attrs'] ) ) {
	...
	public function add_blocks_to_excerpt( $allowed ) {
		$allowed = array_merge( $allowed, apply_filters( 'base_blocks_allowed_excerpt_blocks', array( 'base/advancedheading' ) ) );
	...
	public function add_wrapper_blocks_to_excerpt( $allowed ) {
		$allowed = array_merge( $allowed, apply_filters( 'base_blocks_allowed_excerpt_blocks', array(
			'base/rowlayout',
			'base/column'
		) ) );
		return $allowed;
	...
	public function render_accordion_scheme_head( $accorion ) {
		if( !empty( $accorion['innerBlocks'] ) && is_array( $accorion['innerBlocks'] ) ) {
			foreach( $accorion['innerBlocks'] as $key => $block ) {
				if( isset( $block['blockName'] ) && $block['blockName'] === 'base/pane' ) {
	...
	public function render_pane_scheme_head( $block ) {
		if ( ! is_null( self::$faq_schema ) ) {
			if ( is_array( $block['innerBlocks'] ) && ! empty( $block['innerBlocks'] ) ) {
				...
				preg_match( '/<span class="bst-blocks-accordion-title">(.*?)<\/span>/s', $block['innerHTML'], $match );
	...
	public function faq_schema() {
		...
		$print_faq_schema = apply_filters( 'base_blocks_print_faq_schema', true );
		if ( ! $print_faq_schema ) {
			return;
		}
	...
	public function frontend_gfonts() {
		...
		$print_google_fonts = apply_filters( 'base_blocks_print_google_fonts', true );
	...
	public function frontend_footer_gfonts() {
		...
		$print_google_fonts = apply_filters( 'base_blocks_print_footer_google_fonts', true );
		if ( ! $print_google_fonts ) {
			return;
		}
	...
	public function frontend_build_css( $post_object ) {
		...
		if ( ! method_exists( $post_object, 'post_content' ) ) {
			...
			$base_blocks = apply_filters( 'base_blocks_blocks_to_generate_post_css', array() );
			foreach ( $blocks as $indexkey => $block ) {
				$block = apply_filters( 'base_blocks_frontend_build_css', $block );
				if ( ! is_object( $block ) && is_array( $block ) && isset( $block['blockName'] ) ) {
					if ( isset( $base_blocks[ $block['blockName'] ] ) ) {
						$block_class_instance = $base_blocks[ $block['blockName'] ]::get_instance();
						$block_class_instance->output_head_data( $block );
					}
					/** !!!! Needs to stay to build schema !!! **/
					if ( 'base/accordion' === $block['blockName'] ) {
						...
						if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
							...
							if ( isset( $blockattr['ref'] ) ) {
								...
								if ( $reusable_block && 'wp_block' == $reusable_block->post_type ) {
									...
									$this->blocks_cycle_through( $reuse_data_block, $base_blocks );
								}
							}
						}
					}
					if ( ! empty( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) ) {
						$this->blocks_cycle_through( $block['innerBlocks'], $base_blocks );
					}
				}
			}
		}
	 ...
	 */
	public function blocks_cycle_through( $inner_blocks, $base_blocks ) {
		foreach ( $inner_blocks as $in_indexkey => $inner_block ) {
			$inner_block = apply_filters( 'base_blocks_frontend_build_css', $inner_block );
			if ( ! is_object( $inner_block ) && is_array( $inner_block ) && isset( $inner_block['blockName'] ) ) {
				if ( isset( $base_blocks[ $inner_block['blockName'] ] ) ) {
					$block_class_instance = $base_blocks[ $inner_block['blockName'] ]::get_instance();
					$block_class_instance->output_head_data( $inner_block );
				}
				/** !!!! Needs to stay to build schema !!! **/
				if ( 'base/accordion' === $inner_block['blockName'] ) {
				...
				if ( ! empty( $inner_block['innerBlocks'] ) && is_array( $inner_block['innerBlocks'] ) ) {
					$this->blocks_cycle_through( $inner_block['innerBlocks'], $base_blocks );
				}
			}
		}
	}
...
Base_Blocks_Frontend::get_instance();
