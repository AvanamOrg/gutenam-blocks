...
class Base_Blocks_Svg_Render {
	...
	public function render_icons_dynamically( $block_content, $block ) {
		...
		if ( ! empty( $block_content ) ) {
			$replaced_block_content = preg_replace_callback(
				'/<span\s+((?:data-[\w\-]+=["\']+.*["\']+[\s]+)+)class=["\'].*base-dynamic-icon.*["\']\s*>(.*)<\/span>/U',
				function ( $matches ) {
					...
					return '<span class="bsb-svg-icon-wrap bsb-svg-icon-' . esc_attr( $args['name'] ) . ( ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : '' ) . '">' . $svg . '</span>';
				},
				$block_content
			);
	...
	private static function get_icons() {
		$ico   = include BASE_BLOCKS_PATH . 'includes/icons-ico-array.php';
		$faico = include BASE_BLOCKS_PATH . 'includes/icons-array.php';

		return apply_filters( 'base_svg_icons', array_merge( $ico, $faico ) );
	}

}

Base_Blocks_Svg_Render::get_instance();
