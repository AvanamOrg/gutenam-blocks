<?php
/**
 * REST API Mailchimp controller customized for Base Form
 */
 ...
 */
class Base_MailerLite_REST_Controller extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = 'bsb-mailerlite/v1';
		$this->rest_base = 'get';
	}
	...
	public function get_items( $request ) {
		...
		if ( strlen( $api_key ) > 100 ) {
			...
			$request_args = array(
				...
				'timeout' => 90,
				'user-agent' => 'Base Forms/' . GUTENAM_BLOCKS_VERSION,
				'body' => $args,
			);
		} else {
	...
	public function get_collection_params() {
		...
		$query_params[ self::PROP_API_KEY ] = array(
			'description' => __( 'The API Key for Mailerlite account.', 'gutenam-blocks-pro' ),
			'type'        => 'string',
		);
		...
		$query_params[ self::PROP_QUERY_ARGS ] = array(
			'description' => __( 'Query Args for url.', 'gutenam-blocks-pro' ),
			'type'        => 'array',
		);
