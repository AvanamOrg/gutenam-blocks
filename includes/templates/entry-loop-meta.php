 ...
 *
 * This template can be overridden by copying it to yourtheme/base-blocks/entry-loop-taxonomies.php.
 *
 * @package Base Blocks
 */
...
$author_label                 = ( isset( $attributes ) && is_array( $attributes ) && isset( $attributes['authorLabel'] ) && ! empty( $attributes['authorLabel'] ) ? $attributes['authorLabel'] : __( 'By', 'gutenam-blocks' ) );
...
$date_label                   = ( isset( $attributes ) && is_array( $attributes ) && isset( $attributes['dateLabel'] ) && ! empty( $attributes['dateLabel'] ) ? $attributes['dateLabel'] : __( 'Posted on', 'gutenam-blocks' ) );
...
$date_updated_label           = ( isset( $attributes ) && is_array( $attributes ) && isset( $attributes['dateUpdatedLabel'] ) && ! empty( $attributes['dateUpdatedLabel'] ) ? $attributes['dateUpdatedLabel'] : __( 'Updated on', 'gutenam-blocks' ) );
...
$meta_categories_label        = ( isset( $attributes ) && is_array( $attributes ) && isset( $attributes['metaCategoriesLabel'] ) && ! empty( $attributes['metaCategoriesLabel'] ) ? $attributes['metaCategoriesLabel'] : __( 'Posted in', 'gutenam-blocks' ) );
...
if ( $enabled ) {
	...
	if ( $author ) {
		...
		if ( post_type_supports( $post_type_obj->name, 'author' ) ) {
			...
			if ( $author_link ) {
				if ( get_the_author_meta( 'url', $author_id ) && apply_filters( 'base_author_use_profile_link', true ) ) {
	...
	if ( $meta_categories ) {
		if ( 'post' === get_post_type() ) {
			...
			$separator = _x( ', ', 'list item separator', 'gutenam-blocks' );
			?>
	...
	if ( $meta_comments ) {
		...
		if ( '1' === get_comments_number() ) {
			echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comment', 'gutenam-blocks' );
		} else {
			echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comments', 'gutenam-blocks' );
		}
		echo '</a>';
		echo '</div>';
	}
