...
class BB_Ajax_Form {
	...
	public function __construct() {
		add_action( 'wp_ajax_bsb_process_ajax_submit', array( $this, 'process_ajax' ) );
		add_action( 'wp_ajax_nopriv_bsb_process_ajax_submit', array( $this, 'process_ajax' ) );
	}
	...
	public function process_ajax() {
		if ( isset( $_POST['_bsb_form_id'] ) && ! empty( $_POST['_bsb_form_id'] ) && isset( $_POST['_bsb_form_post_id'] ) && ! empty( $_POST['_bsb_form_post_id'] ) ) {
			$this->start_buffer();
			$valid = true;
			if ( apply_filters( 'base_blocks_form_verify_nonce', false ) ) {
				$valid = check_ajax_referer( 'bsb_form_nonce', '_bsb_form_verify', false );
			}
			if ( $valid ) {
				// Lets get form data.
				$form_id = sanitize_text_field( wp_unslash( $_POST['_bsb_form_id'] ) );
				...
				if ( ! isset( $form_args['email'] ) ) {
					$form_args['email'] = array(
						0 => array(
							...
							'subject' => '[' . get_bloginfo( 'name' ) . ' ' . __( 'Submission', 'gutenam-blocks' ) . ']',
							'replyTo' => 'email_field',
							'html'    => true,
						),
					);
				...
				if ( $honeypot_check_enabled && isset( $_POST['_bsb_verify_email'] ) ) {
				...
				if ( isset( $form_args['recaptcha'] ) && true === $form_args['recaptcha'] ) {
					if ( isset( $form_args['recaptchaVersion'] ) && 'v2' === $form_args['recaptchaVersion'] ) {
						if ( ! isset( $_POST['g-recaptcha-response'] ) || empty( $_POST['g-recaptcha-response'] ) ) {
							$this->process_bail( $messages[0]['recaptchaerror'], __( 'reCAPTCHA Failed', 'gutenam-blocks' ) );
						}
				...
				foreach ( $form_args['fields'] as $key => $data ) {
					// check for required.
					if ( $data['required'] && ( ! isset( $_POST[ 'bsb_field_' . $key ] ) || empty( $_POST[ 'bsb_field_' . $key ] ) ) ) {
						$this->process_bail( $messages[0]['error'], __( 'Required Field Empty', 'gutenam-blocks' ), 'bsb_field_' . $key );
					}
					if ( isset( $_POST[ 'bsb_field_' . $key ] ) ) {
						$fields[ $key ] = array(
							'type'  => $data['type'],
							'label' => str_replace( '{privacy_policy}', $privacy_title, $data['label'] ),
							'value' => $this->sanitize_field( $data['type'], wp_unslash( $_POST[ 'bsb_field_' . $key ] ), $data['multiSelect'] ),
						);
						if ( 'hidden' === $data['type'] ) {
							global $post;
				...
				foreach ( $form_args['actions'] as $data ) {
					switch ( $data ) {
						case 'email':
							...
							$subject       = isset( $form_args['email'][0]['subject'] ) && ! empty( trim( $form_args['email'][0]['subject'] ) ) ? $form_args['email'][0]['subject'] : '[' . get_bloginfo( 'name' ) . ' ' . __( 'Submission', 'gutenam-blocks' ) . ']';
							...
							if ( $form_args['email'][0]['html'] ) {
								...
								$email_content = base_blocks_get_template_html( 'form-email.php', $args );
							} else {
								foreach ( $fields as $key => $data ) {
						...
						case 'redirect':
							if ( isset( $form_args['redirect'] ) && ! empty( trim( $form_args['redirect'] ) ) ) {
								$final_data['redirect'] = apply_filters( 'base_blocks_form_redirect', trim( $form_args['redirect'] ), $form_args, $fields, $form_id, $post_id );
							}
							break;
						case 'mailerlite':
							$api_key = get_option( 'base_blocks_mailerlite_api' );
							if ( empty( $api_key ) ) {
								return;
							}
							...
							if ( isset( $body[ 'email' ] ) ) {
								...
								if ( is_wp_error( $response ) ) {
									...
									if ( ! isset( $response['response'] ) || ! isset( $response['response']['code'] ) ) {
										error_log( __( 'No Response from MailerLite', 'gutenam-blocks' ) );
									}
									if ( 400 === $response['response']['code'] ) {
										error_log( print_r( $response['response'], true ) );
										$this->process_bail( $response['response']['message'] . ' ' . __( 'MailerLite Misconfiguration', 'gutenam-blocks' ), __( 'MailerLite Failed', 'gutenam-blocks' ) );
									}
								}
							}
						break;
				...
				do_action( 'base_blocks_form_submission', $form_args, $fields, $form_id, $post_id );

				$success = apply_filters( 'base_blocks_form_submission_success', true, $form_args, $fields, $form_id, $post_id );
				$messages = apply_filters( 'base_blocks_form_submission_messages', $messages );
				if ( ! $success ) {
					$this->process_bail( $messages[0]['error'], __( 'Third Party Failed', 'gutenam-blocks' ) );
				} else {
					$final_data['html'] = '<div class="base-blocks-form-message base-blocks-form-success">' . $messages[0]['success'] . '</div>';
		...
		if ( strpos( $post_id, 'block-unknown' ) !== false ) {
			...
			if ( is_array( $widget_data ) ) {
				foreach ( $widget_data as $key => $data_array ) {
					if ( ! empty( $data_array['content'] ) && strpos( $data_array['content'], '<!-- wp:base/form {"uniqueID":"' . $form_id . '"' ) !== false ) {
