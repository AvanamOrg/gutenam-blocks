 ...
 * @category class
 */
class Base_Blocks_Table_Of_Contents {
		...
		if ( $multipage ) {
			...
			if ( is_null( self::$the_headings ) ) {
				if ( isset( $attributes['enableDynamicSearch'] ) && $attributes['enableDynamicSearch'] ) {
					if ( isset( $attributes['enableTemplateSearch'] ) && $attributes['enableTemplateSearch'] && class_exists( 'Base_Woo_Block_Editor_Templates' ) && is_singular( 'product' ) ) {
						$base_woo_pro = Base_Woo_Block_Editor_Templates::get_instance();
						$page_content = '';
						if ( ! empty( $base_woo_pro::$single_override ) ) {
							$template_post = get_post( $base_woo_pro::$single_override );
					...
					} else if ( isset( $attributes['enableTemplateSearch'] ) && $attributes['enableTemplateSearch'] && class_exists( 'Base_Pro\Elements_Controller' ) && is_singular() ) {
						$base_pro = Base_Pro\Elements_Controller::get_instance();
						$page_content = '';
						if ( ! empty( $base_pro::$single_template ) ) {
							$blocks = parse_blocks( $post->post_content );
							self::$output_content = '';
							$the_post_content = '';
							foreach ( $blocks as $block ) {
							...
							$template_post = get_post( $base_pro::$single_template );
							$blocks = parse_blocks( $template_post->post_content );
							self::$output_content = '';
							foreach ( $blocks as $block ) {
	...
	private function get_ignore_list() {
		if ( is_null( self::$ignore_list ) ) {
			self::$ignore_list = apply_filters( 'base_toc_block_ignore_array', array( 'base/tableofcontents', 'base/tabs', 'base/modal', 'core/post-content' ) );
		}
	...
	private function get_inner_block_ignore_list() {
		if ( is_null( self::$inner_ignore_list ) ) {
			self::$inner_ignore_list = apply_filters( 'base_toc_inner_block_ignore_array', array( 'base/pane' ) );
		}
	...
	private function get_ignore_classes() {
		if ( is_null( self::$ignore_classes ) ) {
			self::$ignore_classes = apply_filters( 'base_toc_exclude_classes_array', array( 'bst-testimonial-title', 'image-overlay-title', 'image-overlay-subtitle', 'toc-ignore' ) );
		}
	...
	public function headings_enqueue() {
		...
		wp_localize_script(
			'base-blocks-tableofcontents',
			'base_blocks_toc',
			array(
				'headings'      => wp_json_encode( self::$headings ),
				'expandText'    => esc_attr__( 'Expand Table of Contents', 'gutenam-blocks' ),
				'collapseText'  => esc_attr__( 'Collapse Table of Contents', 'gutenam-blocks' ),
			)
		);
	}
	...
	/**
	 * Renders the heading list of the base table of contents block on server.
	 *
	 * @access private
	 *
	...
	private function table_of_contents_render_list( $nested_heading_list, $wrap = false, $list_tag = 'ul' ) {
		$entry_class = 'bsb-table-of-contents__entry';
		...
		return $wrap ? '<' . $list_tag . ' class="bsb-table-of-contents-list-sub">' . implode( $child_nodes ) . '</' . $list_tag . '>' : implode( $child_nodes );
	}
	/**
	...
	public function render_table_of_content( $attributes, $content ) {
		...
		// CSS class string.
		$class = 'wp-block-base-tableofcontents bsb-table-of-content-nav bsb-table-of-content-id' . $attributes['uniqueID'];
		...
		$attributes = apply_filters( 'base_blocks_toc_block_build_only_attributes', $attributes );
		...
		if ( isset( $attributes['enableSmoothScroll'] ) && true === $attributes['enableSmoothScroll'] ) {
			...
			$class .= ' bsb-toc-smooth-scroll';
		}
		$output = '<nav class="' . esc_attr( $class ) . ( $enable_toggle ? ' bsb-collapsible-toc bsb-toc-toggle-' . ( $start_closed ? 'hidden' : 'active' ) : '' ) . '" role="navigation" aria-label="' . esc_attr__( 'Table of Contents', 'gutenam-blocks' ) . '"' . ( $enable_scroll ? ' data-scroll-offset="' . esc_attr( $scroll_offset ) . '"' : '' ) . ( isset( $attributes['enableScrollSpy'] ) && true === $attributes['enableScrollSpy'] ? ' data-scroll-spy="true"' : '' ) . '>';
		$output .= '<div class="bsb-table-of-content-wrap">';
		if ( ! isset( $attributes['enableTitle'] ) || isset( $attributes['enableTitle'] ) && $attributes['enableTitle'] ) {
			$output .= '<div class="bsb-table-of-contents-title-wrap bsb-toggle-icon-style-' . ( $enable_toggle && isset( $attributes['toggleIcon'] ) && $attributes['toggleIcon'] ? $attributes['toggleIcon'] : 'arrow' ) . '">';
			if ( $title_toggle ) {
				$output .= '<button class="bsb-table-of-contents-title-btn bsb-table-of-contents-toggle" aria-expanded="' . esc_attr( $start_closed ? 'false' : 'true' ) . '" aria-label="' . ( $start_closed ? esc_attr__( 'Expand Table of Contents', 'gutenam-blocks' ) : esc_attr__( 'Collapse Table of Contents', 'gutenam-blocks' ) ) . '">';
			}
			$output .= '<span class="bsb-table-of-contents-title">';
			$output .= ( isset( $attributes['title'] ) && $attributes['title'] !== 'Table of Contents' ? $attributes['title'] : __( 'Table of Contents', 'gutenam-blocks' ) );
			$output .= '</span>';
			if ( $enable_toggle ) {
				if ( $title_toggle ) {
					$output .= '<span class="bsb-table-of-contents-icon-trigger"></span>';
				} else {
					$output .= '<button class="bsb-table-of-contents-icon-trigger bsb-table-of-contents-toggle" aria-expanded="' . esc_attr( $start_closed ? 'false' : 'true' ) . '" aria-label="' . ( $start_closed ? esc_attr__( 'Expand Table of Contents', 'gutenam-blocks' ) : esc_attr__( 'Collapse Table of Contents', 'gutenam-blocks' ) ) . '"></button>';
				}
			}
			if ( $title_toggle ) {
				$output .= '</button>';
		...
		$output .= '<' . $list_tag . ' class="bsb-table-of-content-list bsb-table-of-content-list-columns-' . esc_attr( isset( $attributes['columns'] ) && ! empty( $attributes['columns'] ) ? $attributes['columns'] : '1' ) . ' bsb-table-of-content-list-style-' . esc_attr( isset( $attributes['listStyle'] ) && ! empty( $attributes['listStyle'] ) ? $attributes['listStyle'] : 'disc' ) . ' bsb-table-of-content-link-style-' . esc_attr( isset( $attributes['linkStyle'] ) && ! empty( $attributes['linkStyle'] ) ? $attributes['linkStyle'] : 'underline' ) . '">';
		...
		if ( ! wp_style_is( 'base-blocks-table-of-contents', 'enqueued' ) ) {
			wp_enqueue_style( 'base-blocks-table-of-contents' );
		}
		if ( isset( $attributes['uniqueID'] ) ) {
			$unique_id = $attributes['uniqueID'];
			$style_id = 'bsb-tableofcontents' . esc_attr( $unique_id );
			if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'base_blocks_render_inline_css', true, 'tableofcontent', $unique_id ) ) {
				if ( base_blocks_is_not_amp() ) {
					if ( isset( $attributes['enableScrollSpy'] ) && $attributes['enableScrollSpy'] ) {
						wp_enqueue_script( 'base-blocks-gumshoe' );
					}
					wp_enqueue_script( 'base-blocks-table-of-contents' );
				}
				if ( ! doing_filter( 'the_content' ) ) {
					if ( ! wp_style_is( 'base-blocks-table-of-contents', 'done' ) ) {
						wp_print_styles( 'base-blocks-table-of-contents' );
					}
				} else {
					if ( ! wp_style_is( 'base-blocks-table-of-contents', 'done' ) ) {
						ob_start();
...
Base_Blocks_Table_Of_Contents::get_instance();
