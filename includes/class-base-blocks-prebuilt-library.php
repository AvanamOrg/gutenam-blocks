 ...
 */
class Base_Blocks_Prebuilt_Library {
	 ...
	 * @var string
	 */
	protected $remote_url = 'https://patterns.baseblocks.com/wp-json/base-cloud/v1/get/';
	 ...
	 * @var string
	 */
	protected $remote_pages_url = 'https://patterns.baseblocks.com/wp-json/base-cloud/v1/pages/';
	 ...
	 * @var string
	 */
	protected $remote_templates_url = 'https://api.baseblocks.com/wp-json/base-starter/v1/get/';
	...
	public function __construct() {
		if ( is_admin() ) {
			// Ajax Calls.
			add_action( 'wp_ajax_base_import_get_prebuilt_data', array( $this, 'prebuilt_data_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_reload_prebuilt_data', array( $this, 'prebuilt_data_reload_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_get_new_connection_data', array( $this, 'prebuilt_connection_info_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_get_prebuilt_templates_data', array( $this, 'prebuilt_templates_data_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_reload_prebuilt_templates_data', array( $this, 'prebuilt_templates_data_reload_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_get_prebuilt_pages_data', array( $this, 'prebuilt_pages_data_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_reload_prebuilt_pages_data', array( $this, 'prebuilt_pages_data_reload_ajax_callback' ) );
			add_action( 'wp_ajax_base_import_process_data', array( $this, 'process_data_ajax_callback' ) );
			add_action( 'wp_ajax_base_subscribe_process_data', array( $this, 'process_subscribe_ajax_callback' ) );
		}
	...
	public function get_section_prebuilt_data( $pro_data ) {
		...
		if ( empty( $pro_key ) ) {
			$pro_key = ( isset( $pro_data['avanamorg_key'] ) && ! empty( $pro_data['avanamorg_key'] ) ? $pro_data['avanamorg_key'] : '' );
			if ( $pro_key ) {
				$api_email = 'AvanamOrg';
			}
		}
	...
	public function get_template_data( $skip_local = false ) {
		if ( 'custom' === $this->package ) {
			return wp_json_encode( apply_filters( 'base_block_library_custom_array', array() ) );
		}
		...
		// 	}
		// }
		// If it's a Base Pattern Hub connect, lets make sure it's within date.
		if ( 'templates' !== $this->package && 'pages' !== $this->package && 'section' !== $this->package && ! $this->is_template ) {
			$cloud_settings = json_decode( get_option( 'base_blocks_cloud' ), true );
			if ( isset( $cloud_settings['connections'] ) && isset( $cloud_settings['connections'][ $this->package ] ) && isset( $cloud_settings['connections'][ $this->package ]['expires'] ) && ! empty( $cloud_settings['connections'][ $this->package ]['expires'] ) ) {
				...
				if ( $expires < $now ) {
					...
					update_option( 'base_blocks_cloud', json_encode( $cloud_settings ) );
	...
	public function get_local_template_data_filename() {
		...
		if ( class_exists( 'Base_Blocks_Pro' ) ) {
			$bbp_data = base_blocks_get_pro_license_data();
			if ( $bbp_data && isset( $bbp_data['api_key'] ) && ! empty( $bbp_data['api_key'] ) ) {
				$bsb_api = $bbp_data['api_key'];
			}
		}
		if ( 'templates' !== $this->package && 'section' !== $this->package && ! $this->is_template && $this->key ) {
			$bsb_api = $this->key;
		}
		return md5( $this->get_base_url() . $this->get_base_path() . $this->package . GUTENAM_BLOCKS_VERSION . $bsb_api );
	}
	/**
	...
	public function prebuilt_connection_info_ajax_callback() {
		...
		$this->url           = empty( $_POST['url'] ) ? '' : rtrim( sanitize_text_field( $_POST['url'] ), '/' ) . '/wp-json/base-cloud/v1/info/';
		$this->key           = empty( $_POST['key'] ) ? 'section' : sanitize_text_field( $_POST['key'] );
		// Do you have the data?
		$get_data = $this->get_connection_data();
		if ( ! $get_data ) {
			// Send JSON Error response to the AJAX call.
			wp_send_json( esc_html__( 'No Connection data', 'gutenam-blocks' ) );
		} else {
	...
	public function prebuilt_data_ajax_callback() {
		...
		$this->package       = empty( $_POST['package'] ) ? 'section' : sanitize_text_field( $_POST['package'] );
		$this->url           = empty( $_POST['url'] ) ? $this->remote_url : rtrim( sanitize_text_field( $_POST['url'] ), '/' ) . '/wp-json/base-cloud/v1/get/';
		...
		if ( ! $get_data ) {
			// Send JSON Error response to the AJAX call.
			wp_send_json( esc_html__( 'No library data', 'gutenam-blocks' ) );
		} else {
			wp_send_json( $get_data );
		}
		die;
	...
	public function process_subscribe_ajax_callback() {
		...
		if ( $email && is_email( $email ) && filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
			...
			// Get the response.
			$api_url  = add_query_arg( $args, 'https://avanam.org/base-blocks/wp-json/base-subscribe/v1/subscribe/' );
			$response = wp_remote_get(
				$api_url,
				array(
					'timeout' => 20,
	...
	public function process_data_ajax_callback() {
		...
		if ( ! $data ) {
			// Send JSON Error response to the AJAX call.
			wp_send_json( esc_html__( 'No data', 'gutenam-blocks' ) );
		} else {
			wp_send_json( $data );
		}
		die;
	...
	public function import_image( $image_data ) {
		...
		update_post_meta( $post_id, '_base_blocks_image_hash', sha1( $image_data['url'] ) );
		...
		$image_id = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
					WHERE `meta_key` = \'_base_blocks_image_hash\'
						AND `meta_value` = %s
				;',
				sha1( $image_data['url'] )
			)
	...
	public function process_individual_import( $content, $import_library, $import_type, $import_id, $import_style ) {
		if ( isset( $import_library ) && 'pattern' === $import_library ) {
			...
			// Get the response.
			$api_url  = add_query_arg( $args, 'https://patterns.baseblocks.com/wp-json/base-cloud/v1/single/' );
			$response = wp_remote_get(
				$api_url,
				array(
					'timeout' => 20,
	...
	public static function verify_ajax_call() {
		check_ajax_referer( 'base-blocks-ajax-verification', 'security' );
	...
	public function add_monthly_to_cron_schedule( $schedules ) {
		...
		if ( ! isset( $schedules[ self::CLEANUP_FREQUENCY ] ) ) {
			$schedules[ self::CLEANUP_FREQUENCY ] = array(
				...
				'display' => __( 'Once Monthly', 'gutenam-blocks' ),
			);
		}
		return $schedules;
	}
	...
	public function get_old_block_library_folder() {
		$old_block_library_folder = trailingslashit( $this->get_filesystem()->wp_content_dir() ) . 'base_blocks_library';
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'base_block_library_local_data_subfolder_name', 'base_blocks_library' );
		}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			...
			$this->base_path = apply_filters( 'base_block_library_local_data_base_path', trailingslashit( $upload_dir['basedir'] ) );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'base_block_library_local_data_base_url', content_url() );
		}
...
Base_Blocks_Prebuilt_Library::get_instance();
