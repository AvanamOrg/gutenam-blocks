 ...
 */
class Base_Blocks_Google_Fonts {
	/**
	...
	public function frontend_gfonts() {
		...
		if ( class_exists( 'Base_Blocks_Frontend' ) ) {
			$btblocks_instance = Base_Blocks_Frontend::get_instance();
			foreach ( self::$gfonts as $key => $font ) {
				if ( ! array_key_exists( $key, $btblocks_instance::$gfonts ) ) {
					$add_font = array(
					...
					$btblocks_instance::$gfonts[ $key ] = $add_font;
				} else {
					foreach ( $font['fontvariants'] as $variant ) {
						if ( ! in_array( $variant, $btblocks_instance::$gfonts[ $key ]['fontvariants'], true ) ) {
							array_push( $btblocks_instance::$gfonts[ $key ]['fontvariants'], $variant );
						}
					}
				}
			}
		} else {
			$print_google_fonts = apply_filters( 'base_woo_print_google_fonts', true );
	...
	public function frontend_footer_gfonts() {
		...
		if ( class_exists( 'Base_Blocks_Frontend' ) ) {
			...
			foreach ( self::$footer_gfonts as $key => $font ) {
				if ( ! array_key_exists( $key, $btblocks_instance::$footer_gfonts ) ) {
					$add_font = array(
						'fontfamily'   => $font['fontfamily'],
						'fontvariants' => ( isset( $font['fontvariants'] ) && ! empty( $font['fontvariants'] ) && is_array( $font['fontvariants'] ) ? $font['fontvariants'] : array() ),
						'fontsubsets'  => ( isset( $font['fontsubsets'] ) && ! empty( $font['fontsubsets'] ) && is_array( $font['fontsubsets'] ) ? $font['fontsubsets'] : array() ),
					);
					$btblocks_instance::$footer_gfonts[ $key ] = $add_font;
				} else {
					foreach ( $font['fontvariants'] as $variant ) {
						if ( ! in_array( $variant, $btblocks_instance::$footer_gfonts[ $key ]['fontvariants'], true ) ) {
							array_push( $btblocks_instance::$footer_gfonts[ $key ]['fontvariants'], $variant );
						}
					}
				}
			}
		} else {
			$print_google_fonts = apply_filters( 'base_woo_print_footer_google_fonts', true );
			if ( ! $print_google_fonts ) {
				return;
			}
...
Base_Blocks_Google_Fonts::get_instance();
