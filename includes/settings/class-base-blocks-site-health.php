...
if ( ! class_exists( 'Base_Blocks_Site_Health' ) ) {
	/**
	 * Handles Site Health data.
	 *
	 * @since 4.4.1
	 */
	class Base_Blocks_Site_Health {
		private const SITE_HEALTH_KEY = 'gutenam-blocks';
		...
		*/
		...
		public function init_data_settings() {
			if ( ( true !== $this->data_settings_loaded ) ) {
				...
				if ( empty( $this->data_settings['prior_version'] ) ) {
					if ( get_option( 'base_blocks_config_blocks' ) ) {
				...
				if ( ! isset( $this->data_settings['current_version'] ) ) {
					$this->data_settings['current_version'] = GUTENAM_BLOCKS_VERSION;
				}
				if ( ! isset( $this->data_settings['current_version'] ) ) {
					$this->data_settings['current_version'] = 0;
				}
				if ( version_compare( GUTENAM_BLOCKS_VERSION, $this->data_settings['current_version'], 'ne' ) ) {
					...
					$this->data_settings['current_version']           = GUTENAM_BLOCKS_VERSION;
					$this->data_settings['version_history'][ time() ] = GUTENAM_BLOCKS_VERSION;
					$data_settings_changed                 = true;
				}
				...
				if ( true === $data_settings_changed ) {
					...
					update_option( 'baseblocks_data_settings', $this->data_settings );
				}
			}
		}
		/**
		...
		public function add_site_health_info( array $debug_info ): array {
			$debug_info[ self::SITE_HEALTH_KEY ] = array(
				'label'  => __( 'Gutenam Blocks', 'gutenam-blocks' ),
		...
		/**
		 * Maps the debug data to the Site Health fields.
		 *
		 * @return array
		 */
		...
		private function map_general_fields(): array {
			...
			$block_defaults = get_option( 'base_blocks_config_blocks' );
			$block_visibility = get_option( 'base_blocks_settings_blocks' );
			$block_colors = get_option( 'base_blocks_colors' );
			$block_recaptcha = get_option( 'base_blocks_recaptcha_site_key' );
			$mailerlite = get_option( 'base_blocks_mailerlite_api' );
			$maps_api = get_option( 'base_blocks_google_maps_api' );
			$cloud_connect = get_option( 'base_blocks_cloud' );
			...
			return array(
				'version'           => array(
					'label' => __( 'Version', 'gutenam-blocks' ),
					'value' => GUTENAM_BLOCKS_VERSION,
				),
				'last_updated'      => array(
					'label' => __( 'Last updated', 'gutenam-blocks' ),
					'value' => $last_updated > 0 ? $this->adjust_date_time_display( $last_updated ) : __( 'Unknown', 'gutenam-blocks' ),
					'debug' => $last_updated,
				),
				'previous_version'  => array(
					'label' => __( 'Previous version', 'gutenam-blocks' ),
					'value' => $this->get_data_settings( 'prior_version' ),
				),
				'pro_active'  => array(
					'label' => __( 'Has Pro', 'gutenam-blocks' ),
					'value' => ( class_exists( 'Base_Blocks_Pro' ) ? 'Yes' : 'No' ),
				),
				'deactivated_blocks'  => array(
					'label' => __( 'Deactivated Blocks', 'gutenam-blocks' ),
					'value' => ( ! empty( $deactivated_blocks ) ? $deactivated_blocks : __( 'None', 'gutenam-blocks' ) ),
				),
				'block_defaults'  => array(
					'label' => __( 'Has Block Defaults', 'gutenam-blocks' ),
					'value' => $this->check_for_empty_json( $block_defaults ),
				),
				'block_visibility'  => array(
					'label' => __( 'Using Setting Visibility', 'gutenam-blocks' ),
					'value' => $this->check_for_empty_json( $block_visibility ),
				),
				'custom_colors'  => array(
					'label' => __( 'Has Custom Colors', 'gutenam-blocks' ),
					'value' => ( ! empty( $block_colors ) ? 'Yes' : 'No' ),
				),
				'recaptcha_key'  => array(
					'label' => __( 'Has reCAPTCHA key', 'gutenam-blocks' ),
					'value' => ( ! empty( $block_recaptcha ) ? 'Yes' : 'No' ),
				),
				'maps_key'  => array(
					'label' => __( 'Has a maps API key', 'gutenam-blocks' ),
					'value' => ( ! empty( $maps_api ) ? 'Yes' : 'No' ),
				),
				'mailerlite_key'  => array(
					'label' => __( 'Has MailerLite key', 'gutenam-blocks' ),
					'value' => ( ! empty( $mailerlite ) ? 'Yes' : 'No' ),
				),
				'cloud_connect'  => array(
					'label' => __( 'Has Cloud Library', 'gutenam-blocks' ),
		...
		private function check_for_empty_json( $value ) {
			if ( ! isset( $value ) ) {
				return __( 'No', 'gutenam-blocks' );
			} elseif ( empty( $value ) ) {
				return __( 'No', 'gutenam-blocks' );
			} elseif ( '{}' === $value ) {
				return __( 'No', 'gutenam-blocks' );
			}
			return __( 'Yes', 'gutenam-blocks' );
		}
		/**
		...
		private function bool_to_on_off_string( bool $value ): string {
			return $value ? __( 'On', 'gutenam-blocks' ) : __( 'Off', 'gutenam-blocks' );
		}
		...
		private function bool_to_yes_no_string( bool $value ): string {
			return $value ? __( 'Yes', 'gutenam-blocks' ) : __( 'No', 'gutenam-blocks' );
		}
	}
}
Base_Blocks_Site_Health::init();
