...
class AdvancedFormSubmitActions {
	...
	public function email() {
		...
		$subject = isset( $this->form_args['attributes']['email']['subject'] ) && ! empty( trim( $this->form_args['attributes']['email']['subject'] ) ) ? $this->form_args['attributes']['email']['subject'] : '[' . get_bloginfo( 'name' ) . ' ' . __( 'Submission', 'gutenam-blocks' ) . ']';
	...
	public function sendInBlue() {
		$api_key = get_option( 'base_blocks_send_in_blue_api' );
		if ( empty( $api_key ) ) {
			return;
		}
		...
		if ( isset( $body['email'] ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( ! isset( $response['response'] ) || ! isset( $response['response']['code'] ) ) {
					error_log( __( 'No Response from SendInBlue', 'gutenam-blocks-pro' ) );
	...
	public function mailchimp() {
		$api_key = get_option( 'base_blocks_mail_chimp_api' );
		if ( empty( $api_key ) ) {
			return;
		}
		...
		if ( isset( $body['email_address'] ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( ! isset( $response['response'] ) || ! isset( $response['response']['code'] ) ) {
					//error_log( __('Failed to Connect to MailChimp', 'gutenam-blocks-pro' ) );
					return;
				}
	...
	public function convertkit() {
		$api_key            = get_option( 'base_blocks_convertkit_api' );
	...
	public function activecampaign() {
		$api_key  = get_option( 'base_blocks_activecampaign_api_key' );
		$api_base = get_option( 'base_blocks_activecampaign_api_base' );
	...
	public function webhook() {
		...
		$body = array(
			...
			'user_ip'      => $user_ip,
			'user_device'  => ( $browser ? $browser['name'] . '/' . $browser['platform'] : esc_html__( 'Not Collected', 'gutenam-blocks' ) ),
		...
		$args     = apply_filters( 'base_blocks_pro_webhook_args', array( 'body' => $body ) );
	...
	public function entry( $post_id ) {
		...
		$name       = ( isset( $entry_args['name'] ) && ! empty( trim( $entry_args['name'] ) ) ? trim( $entry_args['name'] ) : esc_attr( strip_tags( get_the_title( $this->post_id ) ) ) . ' ' . esc_attr__( 'submission', 'gutenam-blocks-pro' ) );
		...
		$data = array(
			...
			'user_device'  => ( $browser ? $browser['name'] . '/' . $browser['platform'] : esc_html__( 'Not Collected', 'gutenam-blocks-pro' ) ),
		);

		$entries  = new BBP\Queries\Entry();
		$entry_id = $entries->add_item( $data );

		if ( $entry_id ) {
			foreach ( $this->responses as $key => $meta_data ) {
				$response = $this->add_field( $entry_id, 'bsb_field_' . $key, $meta_data );
			}
		}
	}
	...
	public function autoEmail() {
		$auto_defaults = array(
			'subject'   => __( 'Thanks for contacting us!', 'gutenam-blocks-pro' ),
			'message'   => __( 'Thanks for getting in touch, we will respond within the next 24 hours.', 'gutenam-blocks-pro' ),
			'fromEmail' => '',
			'fromName'  => '',
			'replyTo'   => '',
			'cc'        => '',
		...
		$subject         = isset( $auto_email_args['subject'] ) && ! empty( trim( $auto_email_args['subject'] ) ) ? $auto_email_args['subject'] : __( 'Thanks for contacting us!', 'gutenam-blocks-pro' );
		$message         = isset( $auto_email_args['message'] ) && ! empty( trim( $auto_email_args['message'] ) ) ? $auto_email_args['message'] : __( 'Thanks for getting in touch, we will respond within the next 24 hours.', 'gutenam-blocks-pro' );
		...
		if ( ! isset( $auto_email_args['html'] ) || ( isset( $auto_email_args['html'] ) && $auto_email_args['html'] ) ) {
			...
			$email_content = base_blocks_pro_get_template_html( 'form-auto-email.php', $args );
		} else {
	...
	public function add_field( $entry_id, $meta_key, $meta_value, $unique = false ) {
		...
		return add_metadata( 'bbp_form_entry', $entry_id, $meta_key, $meta_value, $unique );
	}
