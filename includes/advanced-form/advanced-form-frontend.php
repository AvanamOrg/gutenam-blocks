...
class AdvancedFormFrontend {
	...
	const ADVANCED_FORM_BLOCK_PREFIX = 'base/advanced-form-';

	const HELP_CLASS_NAME = 'bsb-form-field-help';

	const REQUIRED_CLASS_NAME = 'bsb-advanced-form-required';

	const LABEL_CLASS_NAME = 'bsb-advanced-form-label';
	...
	public function render() {
		$this->append_response( '<div class="wp-block-base-advanced-form wp-block-base-advanced-form' . $this->unique_id . '">' );
		$this->append_response( '<form method="post" class="bsb-advanced-form">' );

		if ( is_numeric( $this->post_id ) && 0 !== $this->post_id ) {
			$this->append_response( '<input type="hidden" name="_bsb_form_post_id" value="' . $this->post_id . '">' );
		}

		$this->append_response( '<input type="hidden" name="_bsb_form_id" value="' . $this->unique_id . '">' );
		$this->append_response( '<input type="hidden" name="action" value="bsb_process_advanced_form_submit">' );


		// Honeypot
		$this->append_response( '<input class="base-blocks-field verify" type="hidden" name="_bsb_verify_email" autoComplete="off" placeholder="Email" tabIndex="-1" />' );
	...
	private function submit_button() {
		...
		$desktop_width = ! empty( $this->parentAttributes['submit']['width'][0] ) ? 'bsb-field-desk-width-' . $this->parentAttributes['submit']['width'][0] : '';
		$tablet_width  = ! empty( $this->parentAttributes['submit']['width'][1] ) ? 'bsb-field-tablet-width-' . $this->parentAttributes['submit']['width'][1] : '';
		$mobile_width  = ! empty( $this->parentAttributes['submit']['width'][2] ) ? 'bsb-field-mobile-width-' . $this->parentAttributes['submit']['width'][2] : '';

		$width_classes = trim( $desktop_width . ' ' . $tablet_width . ' ' . $mobile_width );

		$this->append_response( '<div class="bsb-advanced-form-submit-container ' . $width_classes . ' ">' );
		$this->append_response( '<input type="submit" value="' . $label . '" class="bsb-advanced-form-submit-button">' );
		$this->append_response( '</div>' );
	}
	...
	private function text_field( $block, $type ) {
		...
		$this->append_response( '<div class="bsb-advanced-form-field" class="' . $this->field_width_classes( $block ) . '">' );

		$this->field_label( $block );

		$this->append_response( '<input name="bsb_field_1" id="bsb_field__04ea80-5f_1" ' . $this->aria_described_by( $block ) . ' data-label="' . $this->get_label( $block ) . '" type="' . $type . '" placeholder="' . $this->get_placeholder( $block ) . '" value="" data-type="' . $type . '" class="bsb-field bsb-' . $type . '-field" data-required="' . $is_required . '" />' );
	...
	private function checkbox_field( $block ) {
		...
		foreach ( $block['attrs']['options'] as $key => $option ) {
			...
			$this->append_response( '<div>' );
			$this->append_response( '<input class="bsb-radio-style" type="checkbox" ' . $this->aria_described_by( $block ) . ' id="' . $id . '" name="' . $name . '" '. ($is_checked ? "checked" : "") .' value="' . $this->get_option_value( $option ) . '" ' . $is_required . '>' );
	...
	private function radio_field( $block ) {
		...
		foreach ( $block['attrs']['options'] as $key => $option ) {
			...
			$this->append_response( '<div>' );
			$this->append_response( '<input class="bsb-radio-style" type="radio" ' . $this->aria_described_by( $block ) . ' ' . ( $is_checked ? 'checked' : '') . ' id="' . $id . '" name="' . $name . '" value="' . $this->get_option_value( $option ) . '" ' . $is_required . '>' );
			$this->append_response( '<label for="' . $id . '">' . $option['label'] . '</label>' );
			$this->append_response( '</div>' );
		}
	...
	private function file_field( $block ) {
		...
		$this->append_response( '<input class="' . $this->field_width_classes( $block ) . '" name="bsb_field_1" id="bsb_field__04ea80-5f_1" ' . $this->aria_described_by( $block ) . ' data-label="' . $this->get_label( $block ) . '" type="file" data-type="file" data-required="' . $is_required . '" />' );
	...
	private function textarea_field( $block ) {
		...
		$this->append_response( '<textarea rows="' . $rows . '" class="' . $this->field_width_classes( $block ) . '" ' . $this->aria_described_by( $block ) . ' placeholder="' . $this->get_placeholder( $block ) . '" name="bsb_field_1" id="bsb_field__04ea80-5f_1" data-label="' . $this->get_label( $block ) . '" data-required="' . $is_required . '"></textarea>' );
	...
	private function field_label( $block ) {
		...
		if ( ! empty( $this->get_label( $block ) ) ) {
			$this->append_response( '<label class="' . self::LABEL_CLASS_NAME . '" for="bsb_field__04ea80-5f_1">' . $this->get_label( $block ) );
	...
	private function field_width_classes( $block ) {
		...
		if ( ! empty( $block['attrs']['width'][0] ) && ( is_numeric( $block['attrs']['width'][0] ) || $block['attrs']['width'][0] === 'unset' ) ) {
			$classes .= 'bsb-field-desk-width-' . $block['attrs']['width'][0];
		} elseif ( empty( $block['attrs']['width'][0] ) ) {
			$classes .= 'bsb-field-desk-width-100';
		}
		...
		if ( ! empty( $block['attrs']['width'][1] ) && ( is_numeric( $block['attrs']['width'][1] ) || $block['attrs']['width'][1] === 'unset' ) ) {
			...
			if ( $block['attrs']['width'][0] !== $block['attrs']['width'][1] ) {
				$classes .= ' bsb-field-desk-width-' . $block['attrs']['width'][1];
			}
		}

		// Mobile
		if ( ! empty( $block['attrs']['width'][2] ) && ( is_numeric( $block['attrs']['width'][2] ) || $block['attrs']['width'][2] === 'unset' ) ) {
			$classes .= ' bsb-field-desk-width-' . $block['attrs']['width'][2];
		}
