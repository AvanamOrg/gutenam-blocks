...
import { TypographyControls, MeasurementControls, PopColorControl } from '@base/components';
...
class BaseGlobalTypography extends Component {
	constructor() {
		super( ...arguments );
		...
		this.state = {
			...
			globalStyles: ( base_blocks_params.global ? JSON.parse( base_blocks_params.global ) : {} ),
			user: ( base_blocks_params.user ? base_blocks_params.user : 'admin' ),
		};
	}
	componentDidMount() {
		const typographyConfig = ( this.state.globalStyles && this.state.globalStyles[ 'typography' ] ? this.state.globalStyles[ 'typography' ] : {} );
		if ( typographyConfig.p && typographyConfig.p[0] ) {
			if ( typographyConfig.p[ 0 ].padding[ 0 ] === typographyConfig.p[ 0 ].padding[ 1 ] && typographyConfig.p[ 0 ].padding[ 0 ] === typographyConfig.p[ 0 ].padding[ 2 ] && typographyConfig.p[ 0 ].padding[ 0 ] === typographyConfig.p[ 0 ].padding[ 3 ] ) {
				this.setState( { pPaddingControl: 'linked' } );
			} else {
				this.setState( { pPaddingControl: 'individual' } );
			}
			if ( typographyConfig.p[ 0 ].margin[ 0 ] === typographyConfig.p[ 0 ].margin[ 1 ] && typographyConfig.p[ 0 ].margin[ 0 ] === typographyConfig.p[ 0 ].margin[ 2 ] && typographyConfig.p[ 0 ].margin[ 0 ] === typographyConfig.p[ 0 ].margin[ 3 ] ) {
	...
	saveConfig( settingID, settingArray ) {
		...
		const config = ( base_blocks_params.global ? JSON.parse( base_blocks_params.global ) : {} );
		if ( ! config[ settingID ] ) {
			config[ settingID ] = {};
		}
		config[ settingID ] = settingArray;
		const settingModel = new wp.api.models.Settings( { base_blocks_global: JSON.stringify( config ) } );
		settingModel.save().then( response => {
			this.setState( { isSaving: false, globalStyles: config } );
			base_blocks_params.global = JSON.stringify( config );
		} );
	}
	...
	render() {
		...
		const pStyles = ( undefined !== typographyConfig && undefined !== typographyConfig.p && undefined !== typographyConfig.p[ 0 ] ? typographyConfig.p : pDefaultStyles );
		...
		return (
			<Fragment>
				<div className="bsb-global-typography-container">
					<div className="bsb-global-typography-label">
						<h2 className="bst-beside-color-label">{ __( 'Body' ) }</h2>
						<ToggleControl
					...
					{ pStyles[ 0 ].enable && (
						<div className="bst-inner-sub-section">
							...
							>
								...
								/>
								<ButtonGroup className="bst-size-type-options bst-row-size-type-options" aria-label={ __( 'Padding Type', 'gutenam-blocks' ) }>
...
export default BaseGlobalTypography;
