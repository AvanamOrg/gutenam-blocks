...
function BuildPageContent( rows ) {
	...
	Object.keys( rows ).map( function( key, index ) {
		...
		const rowStart = `<!-- wp:base/column {"borderWidth":["","","",""],"uniqueID":"_f8d4f6-${key}","borderStyle":[{"top":["","",""],"right":["","",""],"bottom":["","",""],"left":["","",""],"unit":"px"}],"className":"bsb-pattern-preview-${rowStyle}"} --><div class="wp-block-base-column base-column_f8d4f6-${key} bsb-pattern-preview-${rowStyle}"><div class="bst-inside-inner-col">`;
		const rowEnd = `</div></div><!-- /wp:base/column -->`;
...
function PageList( {
	...
	const filteredBlockPatterns = useMemo( () => {
		...
		Object.keys( pages ).map( function( key, index ) {
			...
			temp['locked'] = ( pages[key].pro && 'true' !== base_blocks_params.pro ? true : false );
	...
	const customStyles = useMemo( () => {
		...
		if ( ! base_blocks_params.isBaseT ) {
			globalColors = `
				--global-palette1:${base_blocks_params.global_colors['--global-palette1']};
				--global-palette2:${base_blocks_params.global_colors['--global-palette2']};
				--global-palette3:${base_blocks_params.global_colors['--global-palette3']};
				--global-palette4:${base_blocks_params.global_colors['--global-palette4']};
				--global-palette5:${base_blocks_params.global_colors['--global-palette5']};
				--global-palette6:${base_blocks_params.global_colors['--global-palette6']};
				--global-palette7:${base_blocks_params.global_colors['--global-palette7']};
				--global-palette8:${base_blocks_params.global_colors['--global-palette8']};
				--global-palette9:${base_blocks_params.global_colors['--global-palette9']};
			`;
		...
		newStyles = [
			{ css: `:root {margin-block:0;}body{${normalizeStyles} ${globalColors}}.single-iframe-content .bsb-pattern-delete-block {display: none;}${colorClasses}` }
		];
		return newStyles;
	}, [ selectedStyle ] );
	// Define selected style.
	const customShadowStyles = useMemo( () => {
		...
		const normalizeStyles = `--global-content-edge-padding: 3rem;padding:0px !important;`;
		newStyles = [
			{ css: `.single-iframe-content{${normalizeStyles} ${globalColors}}.pattern-shadow-wrap .single-iframe-content {--global-content-width:1200px }.single-iframe-content .bsb-pattern-delete-block {display: none;}${colorClasses}` }
		];
		return newStyles;
	}, [ selectedStyle ] );
