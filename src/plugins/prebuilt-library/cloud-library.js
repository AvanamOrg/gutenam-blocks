...
class CloudSections extends Component {
	...
	loadTemplateData() {
		...
		if ( action === undefined || action.length == 0 ) {
			const cloudSettings = base_blocks_params.cloud_settings ? JSON.parse( base_blocks_params.cloud_settings ) : {};
		...
		if ( action === undefined || action.length == 0 ) {
			if ( typeof base_blocks_params.prebuilt_libraries === 'object' && base_blocks_params.prebuilt_libraries !== null ) {
				this.setState( { actions: base_blocks_params.prebuilt_libraries.concat( this.state.actions ) } );
				action = base_blocks_params.prebuilt_libraries.filter( obj => {
					return obj.slug === this.props.tab;
				});
			}
		}
	...
	render() {
		...
		return (
			<div className={ `bst-prebuilt-content${ ( sidebarEnabled === 'show' ? ' bsb-prebuilt-has-sidebar' : '' ) }` }>
				{ sidebarEnabled === 'show' && (
					<div className="bst-prebuilt-sidebar bsb-cloud-library-sidebar">
						...
						<div className="bsb-library-sidebar-bottom">
							{ sideCatOptions.map( ( category, index ) =>
								<Button
									...
									className={ 'bsb-category-button' + ( getActiveCat === category.value ? ' is-pressed' : '' ) }
				...
				{ ( this.state.isImporting || this.state.isLoading || false === libraryItems || this.state.errorItems ) ? (
					...
					<div className='bsb-cloud-library-outer-wrap'>
						<Masonry
							breakpointCols={breakpointColumnsObj}
							className={ `bsb-css-masonry bsb-cloud-library-wrap` }
							columnClassName="bsb-css-masonry_column"
							// className={ 'bsb-prebuilt-grid bsb-prebuilt-masonry-grid' }
							// elementType={ 'div' }
							// options={ {
										...
										<div className="bsb-css-masonry-inner">
											...
											{ undefined !== pro && pro && (
												<Fragment>
													...
													{ locked && (
														<div className="bsb-popover-pro-notice">
															<h2>{ __( 'Pro required for this item', 'gutenam-blocks' ) } </h2>
														</div>
													) }
												</Fragment>
											) }
