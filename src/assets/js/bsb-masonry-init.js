(function() {
	'use strict';
	window.baseBlocksMasonry = {
		trigger_animation_class: function( element ) {
			element.classList.add( 'bst-masonry-trigger-animation' );
		},
		init: function() {
			var masonryitems = document.querySelectorAll( '.bsb-masonry-init' );
			...
			for ( let i = 0; i < masonryitems.length; i++ ) {
				...
				// 		subitems[n].classList.add( 'bst-masonry-trigger-animation' );
				// 	}, n * 75, n );
				// }
	...
	if ( 'loading' === document.readyState ) {
		...
		document.addEventListener( 'DOMContentLoaded', window.baseBlocksMasonry.init );
	} else {
		// The DOM has already been loaded.
		window.baseBlocksMasonry.init();
	}
})();