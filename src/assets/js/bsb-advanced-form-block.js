/* global base_blocks_advanced_form_params */
( function() {
	'use strict';
	window.baseAdvancedForm = {
		error_item: 1,
		clearForm( form ) {
			form.reset();
		},
		...
		markError( item, error_type, form ) {
			...
			if ( error_type ) {
				...
				if ( item.classList.contains( 'bsb-checkbox-style' ) ) {
					...
					window.baseAdvancedForm.insertAfter( el, item );
				}
			}
			if ( 1 === window.baseAdvancedForm.error_item ) {
				item.focus();
			}
			window.baseAdvancedForm.error_item ++;
		},
		addErrorNotice( form ) {
			...
			window.baseAdvancedForm.insertAfter( el, form );
		},
		isValidEmail( email ) {
		...
		validateForm( self ) {
			...
			window.baseAdvancedForm.removeErrors( self );
					...
					switch( data_type ) {
						...
						case 'text':
							...
							if ( val === '') {
								...
								window.baseAdvancedForm.markError( required[n], error_type, self );
							}
							break;
						case 'tel':
						...
						case 'email':
							...
							if ( val !== '' ) {
								//run the validation
								if( ! window.baseAdvancedForm.isValidEmail( val ) ) {
									error = true;
						...
						case 'url':
							...
							if ( val !== '' ) {
								//run the validation
								if( ! window.baseAdvancedForm.isValidURL( val ) ) {
									error = true;
									error_type = 'validation';
			...
			if ( error ) {
				// add error notice
				window.baseAdvancedForm.addErrorNotice( self );
			...
			//form_data = window.baseAdvancedForm.serialize( form_data );
		...
		submit( e, form ) {
			...
			var submitButton = form.querySelector('.bsb-advanced-form-submit-button');
			var form_data = window.baseAdvancedForm.validateForm( form );
			if ( form_data ) {
				var el = document.createElement('div');
				...
				request.onload = function () {
					if ( this.status >= 200 && this.status < 400 ) {
						...
						if ( response.success ) {
							var event = new CustomEvent( 'bsb-advanced-form-success', {
							...
							if ( response.redirect ) {
								...
								window.baseAdvancedForm.insertAfter( window.baseAdvancedForm.createElementFromHTML( response.html ), form );
								if ( form.querySelector('.g-recaptcha') ) {
									grecaptcha.reset();
								}
								window.baseAdvancedForm.clearForm( form );
							}
						} else {
							if ( response.data ) {
								window.baseAdvancedForm.insertAfter( window.baseAdvancedForm.createElementFromHTML( response.data.html ), form );
								if ( response.data.required ) {
									if ( form.querySelector( '[name="' + response.data.required + '"]' ) ) {
										window.baseAdvancedForm.markError( form.querySelector( '[name="' + response.data.required + '"]' ), 'required', form );
									}
								}
							}
						}
		...
		checkParentClass( element, classname ) {
			...
			return element.parentNode && window.baseAdvancedForm.checkParentClass( element.parentNode, classname );
		},
		verifySource( form ) {
			...
			if ( ! input.value || 'block-unknown' === input.value ) {
				var theID = window.baseAdvancedForm.checkParentClass( form.parentNode, 'widget_block' );
				if ( theID ) {
					input.value = theID;
				}
			}
		...
		initForms() {
			var forms = document.querySelectorAll( 'form.bsb-advanced-form' );
			if ( ! forms.length ) {
				return;
			}
			var click_function = function( form ) {
				return function curried_func(e) {
					window.baseAdvancedForm.submit( e, form );
				}
			}
			for ( var n = 0; n < forms.length; n++ ) {
				window.baseAdvancedForm.verifySource( forms[n] );
		...
		init: function() {
			...
			window.baseAdvancedForm.initForms();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.baseAdvancedForm.init );
	} else {
		// The DOM has already been loaded.
		window.baseAdvancedForm.init();
	}
}() );
