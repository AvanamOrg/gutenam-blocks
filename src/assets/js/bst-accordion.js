...
/*#__PURE__*/
function () {
  function BaseAccordion(el, options) {
    var _this2 = this;
    ...
    var defaults = {
      headerClass: '.bst-blocks-accordion-header',
      panelClass: '.bst-accordion-panel',
      panelInnerClass: '.bst-accordion-panel-inner',
      hiddenClass: 'bst-accordion-panel-hidden',
	  activeClass: 'bst-accordion-panel-active',

      get hidenClass() {
        return this.hiddenClass;
      },

      initializedClass: 'bst-accordion-initialized',

      get initalisedClass() {
        return this.initializedClass;
      },

      headerDataAttr: 'data-bst-accordion-header-id',
  ...
  _createClass(BaseAccordion, [{
      ...
      if (animationAction !== undefined && headerIndex !== undefined) {
        if ( animationAction === 'closed' ) {
          ...
          if ( ! panelToClose.classList.contains(this.settings.hiddenClass) ) {
            ...
            panelToClose.classList.add('bst-panel-is-collapsing');
            ...
            setTimeout(function(){
              ...
              panelToClose.classList.remove('bst-panel-is-collapsing');
          ...
          if ( ! panelToOpen.classList.contains( this.settings.activeClass ) ) {
            ...
            panelToOpen.classList.add('bst-panel-is-expanding');
            ...
            setTimeout(function(){
              ...
              panelToOpen.classList.remove('bst-panel-is-expanding');
            return _this8.toggling = false;
            }, _transDuration );
          }
    ...
    value: function _setupHeaders() {
      ...
      this.headers.forEach(function (header, index) {
        header.setAttribute('id', "bst-accordion-header-".concat(_this12.ids[index].id));
        header.setAttribute('aria-controls', "bst-accordion-panel-".concat(_this12.ids[index].id));
      });
    }
    /**
     * SET UP PANELS
    ...
    value: function _setupPanels() {
      ...
      this.panels.forEach(function (panel, index) {
        panel.setAttribute( 'id', 'bst-accordion-panel-'.concat(_this13.ids[index].id) );
        panel.setAttribute( 'aria-labelledby', 'bst-accordion-header-'.concat(_this13.ids[index].id) );
...
(function() {
  'use strict';
  var hasInitializedBaseAccordion = false;
	window.BaseBlocksAccordion = {
    /**
      ...
      if ( window.location.hash != '' ) {
        ...
        if ( e.type && e.type === 'initialized' && hasInitializedBaseAccordion ) {
          return;
        }
        hasInitializedBaseAccordion = true;
        element = document.getElementById( id );
        if ( element ) {
          if ( element.classList.contains('wp-block-base-pane') ) {
            var child = document.querySelectorAll('#' + id + ' .bst-blocks-accordion-header')[0];
            if ( ! child.classList.contains( 'bst-accordion-panel-active' ) ) {
            ...
            // if ( e.type && e.type === 'initialized' ) {
            //   window.setTimeout(function() {
            //     window.BaseBlocksAccordion.scroll( document.body, document.getElementById( id ).offsetTop, 600 );
            //   }, 350 );
            // }
          }
        }
		...
		init: function() {
			var accordions = document.querySelectorAll('.bst-accordion-inner-wrap');
      ...
      for (var i = 0, len = accordionsArray.length; i < len; i++) {
        ...
        if ( openPanels !== 'none' ) {
          for (var b = 0, lenb = accordionsArray[i].children.length; b < lenb; b++) {
            if ( accordionsArray[i].children[b].classList.contains( 'bst-accordion-pane-' + parseInt( 1 + openPanel ) ) ) {
              openPanel = b;
              break;
            }
          }
        }
        accordionsArray[i].addEventListener( 'initialized', window.BaseBlocksAccordion.anchor, false );
        new BaseAccordion( accordionsArray[i], {
      ...
      window.addEventListener( 'hashchange', window.BaseBlocksAccordion.anchor, false );
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.BaseBlocksAccordion.init );
	} else {
		// The DOM has already been loaded.
		window.BaseBlocksAccordion.init();
	}
})();