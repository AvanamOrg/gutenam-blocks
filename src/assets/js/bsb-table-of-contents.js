/* global base_blocks_toc */
/**
 * File bsb-table-of-contents.js.
...
( function() {
	...
	window.baseTOC = {
		...
		initAddAnchors: function() {
			var headings = JSON.parse( base_blocks_toc.headings );
		...
		initCollapse: function() {
			var collapse_items = document.querySelectorAll( '.bsb-collapsible-toc' );
			if ( ! collapse_items.length ) {
				return;
			}
			for ( let n = 0; n < collapse_items.length; n++ ) {
				var el = collapse_items[n].querySelector( '.bsb-table-of-contents-toggle' );
				el.onclick = () => {
					window.baseTOC.toggleAttribute( el, 'aria-expanded', 'true', 'false' );
					window.baseTOC.toggleAttribute( el, 'aria-label', base_blocks_toc.collapseText, base_blocks_toc.expandText );
					window.baseTOC.toggleClass( collapse_items[n], 'bsb-toc-toggle-active', 'bsb-toc-toggle-hidden' );
				}
			}
		},
		 ...
		 */
		initScroll: function() {
			var scroll_toc = document.querySelectorAll( '.bsb-toc-smooth-scroll' );
			if ( ! scroll_toc.length ) {
				return;
			}
			for ( let n = 0; n < scroll_toc.length; n++ ) {
				var offset = parseInt( scroll_toc[n].getAttribute( 'data-scroll-offset' ) );
				var elements = scroll_toc[n].querySelectorAll( 'a.bsb-table-of-contents__entry' );
				for ( let i = 0; i < elements.length; i++ ) {
					elements[i].onclick = ( e ) => {
						...
						e.preventDefault();
						window.baseTOC.scrollToElement( targetAnchor, offset );
					};
				}
			}
		},
		initScrollSpy: function() {
			if ( typeof Gumshoe == 'function' ) {
				var scroll_spy = document.querySelectorAll( '.wp-block-base-tableofcontents[data-scroll-spy="true"]' );
				...
				for ( let n = 0; n < scroll_spy.length; n++ ) {
					...
					// Initialize Gumshoe
					spy_item[ n ] = new Gumshoe( '.'  + scroll_spy[n].classList[2] + ' .bsb-table-of-content-list a', {
						nested: true,
		...
		init: function() {
			window.baseTOC.initAddAnchors();
			window.baseTOC.initCollapse();
			window.baseTOC.initScroll();
			window.baseTOC.initScrollSpy();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.baseTOC.init );
	} else {
