...
class MailerLiteControls extends Component {
	...
	componentDidMount() {
		...
		apiFetch( {
			...
			this.setState( {
				api: response.base_blocks_mailerlite_api,
			} );
			if ( '' !== this.state.api ) {
	...
	getMailerLiteGroup() {
		let errorNote = __( 'Groups could not be accessed using API key', 'gutenam-blocks' );
		if ( ! this.state.api ) {
		...
		apiFetch( {
			...
			.then( ( groups ) => {
				if ( Array.isArray( groups ) ) {
					...
					if ( 'Forbidden' === groups ) {
						errorNote = __( 'Retrieving groups was denied (403) for that API token or because of Mailerlite firewall. Contact MailerLite for support.', 'gutenam-blocks' );
					} else if ( 'Unauthorized' === groups ) {
						errorNote = __( 'Retrieving groups was denied because the provided API token is invalid.', 'gutenam-blocks' );
					}
	...
	saveAPI() {
		this.setState( { isSaving: true } );
		const settingModel = new wp.api.models.Settings( {
			base_blocks_mailerlite_api: this.state.api,
		} );
		...
		return (
			...
			>
				...
				<div className="components-base-control">
					...
					>
						{ this.state.isSaving ? __( 'Saving', 'gutenam-blocks-pro' ) : __( 'Save', 'gutenam-blocks-pro' ) }
					</Button>
