...
const v5 = {
	...
	save: ( { attributes } ) => {
		...
		const firstColumnClass = ( firstColumnWidth && ( 2 === columns || 3 === columns ) ? ' bst-custom-first-width-' + firstColumnWidth : '' );
		const secondColumnClass = ( secondColumnWidth && ( 2 === columns || 3 === columns ) ? ' bst-custom-second-width-' + secondColumnWidth : '' );
		const thirdColumnClass = ( secondColumnWidth && firstColumnWidth && 3 === columns ? ' bst-custom-third-width-' + ( Math.round( ( 100 - ( parseFloat( firstColumnWidth ) + parseFloat( secondColumnWidth ) ) ) * 10 ) / 10 ) : '' );
		...
		const innerClasses = classnames( 'bst-row-layout-inner', {
			'bst-row-has-bg': bgColor || bgImg || overlay || overlayBgImg,
			[ 'bst-layout-id' + classId ]: classId,
			'bst-jarallax': bgImg && bgImgAttachment === 'parallax',
			[ backgroundColorClass ]: backgroundColorClass,
		} );
		const innerColumnClasses = classnames( {
			'bst-row-column-wrap': true,
			[ `bst-has-${ columns }-columns` ]: columns,
			[ `bst-gutter-${ columnGutter }`]: columnGutter,
			[ `bst-v-gutter-${ ( collapseGutter ? collapseGutter : 'default' ) }`]: true,
			[ `bst-row-valign-${ verticalAlignment }`]: verticalAlignment,
			[ `bst-row-layout-${ layoutClass }` ]: layoutClass,
			...
			'bst-inner-column-height-full': ( undefined !== columnsInnerHeight && true === columnsInnerHeight ),
			...
			return (
				<div className="bsb-bg-slide-contain">
					<div className={ `bsb-bg-slide bsb-bg-slide-${ index }` } style={ {
						backgroundColor: ( bgSlider[ index ] && '' !== bgSlider[ index ].bgColor ? BaseColorOutput( bgSlider[ index ].bgColor ) : undefined ),
		...
		return (
			<HtmlTagOut className={ classes }>
				<div id={ `bst-layout-id${ uniqueID }` } className={ innerClasses } style={ {
					...
					{ ( 'video' === backgroundSettingTab ) && (
						<div className={ 'bsb-blocks-bg-video-container' }>
					...
					{ hasOverlay && (
						<div className={ `bst-row-layout-overlay bst-row-overlay-${ overlayType }${ overlayBgImg && 'gradient' !== overlayType && overlayBgImgAttachment === 'parallax' ? ' bst-jarallax' : '' }` }></div>
					) }
					{ topSep && 'none' !== topSep && '' !== topSep && (
						<div className={ `bst-row-layout-top-sep bst-row-sep-type-${ topSep }` }>
							<svg style={ { fill: BaseColorOutput( topSepColor ) } } viewBox="0 0 1000 100" preserveAspectRatio="none">
								{ topSVGDivider }
							</svg>
						</div>
					) }
					...
					{ bottomSep && 'none' !== bottomSep && '' !== bottomSep && (
						<div className={ `bst-row-layout-bottom-sep bst-row-sep-type-${ bottomSep }` }>
							<svg style={ { fill: BaseColorOutput( bottomSepColor ) } } viewBox="0 0 1000 100" preserveAspectRatio="none">
								{ bottomSVGDivider }
							</svg>
						</div>
					) }
...
const v4 = {
	...
	save: ( { attributes } ) => {
		...
		//const innerColumnClasses = classnames( `bst-row-column-wrap bst-has-${ columns }-columns bst-gutter-${ columnGutter } bst-v-gutter-${ ( collapseGutter ? collapseGutter : 'default' ) } bst-row-valign-${ verticalAlignment } bst-row-layout-${ layoutClass } bst-tab-layout-${ tabLayoutClass } bst-m-colapse-${ collapseOrder } bst-mobile-layout-${ mobileLayoutClass }${ firstColumnClass }${ secondColumnClass }${ thirdColumnClass }${ ( undefined !== columnsInnerHeight && true === columnsInnerHeight ? ' bst-inner-column-height-full' : '' ) }` );
		const innerColumnClasses = classnames( {
			'bst-row-column-wrap': true,
			[ `bst-has-${ columns }-columns` ]: columns,
			[ `bst-gutter-${ columnGutter }`]: columnGutter,
...
const v3 = {
	...
	save: ( { attributes } ) => {
		const { columns, blockAlignment, mobileLayout, currentOverlayTab, overlayBgImg, overlay, colLayout, tabletLayout, collapseOrder, uniqueID, columnGutter, collapseGutter, bgColor, bgImg, verticalAlignment, htmlTag, bottomSep, bottomSepColor, topSep, topSepColor, firstColumnWidth, secondColumnWidth, overlayBgImgAttachment, bgImgAttachment, columnsInnerHeight, backgroundInline, backgroundSettingTab, backgroundSliderCount, backgroundSliderSettings, backgroundSlider, bgImgSize, bgImgPosition, bgImgRepeat, backgroundVideoType, backgroundVideo } = attributes;
		let bottomSVGDivider;
		if ( 'ct' === bottomSep ) {
			bottomSVGDivider = <path d="M1000,0l-500,98l-500,-98l0,100l1000,0l0,-100Z" />;
		} else if ( 'cti' === bottomSep ) {
		...
		const mobileLayoutClass = ( ! mobileLayout ? 'inherit' : mobileLayout );
		const classId = ( ! uniqueID ? 'notset' : uniqueID );
		const hasBG = ( bgColor || bgImg || overlay || overlayBgImg ? 'bst-row-has-bg' : '' );
		const overlayType = ( ! currentOverlayTab || 'grad' !== currentOverlayTab ? 'normal' : 'gradient' );
		const classes = classnames( `align${ ( blockAlignment ? blockAlignment : 'none' ) }` );
		const innerClasses = classnames( `bst-row-layout-inner ${ hasBG } bst-layout-id${ classId }${ bgImg && bgImgAttachment === 'parallax' ? ' bst-jarallax' : '' }` );
		const innerColumnClasses = classnames( `bst-row-column-wrap bst-has-${ columns }-columns bst-gutter-${ columnGutter } bst-v-gutter-${ ( collapseGutter ? collapseGutter : 'default' ) } bst-row-valign-${ verticalAlignment } bst-row-layout-${ layoutClass } bst-tab-layout-${ tabLayoutClass } bst-m-colapse-${ collapseOrder } bst-mobile-layout-${ mobileLayoutClass }${ firstColumnClass }${ secondColumnClass }${ thirdColumnClass }${ ( undefined !== columnsInnerHeight && true === columnsInnerHeight ? ' bst-inner-column-height-full' : '' ) }` );
		const renderSliderImages = ( index ) => {
			let bgSlider;
			if ( undefined === backgroundSlider || ( undefined !== backgroundSlider && undefined === backgroundSlider[ 0 ] ) ) {
		...
		const renderVideo = () => {
			...
			return (
				<Fragment>
					...
					{ true === bgVideo[ 0 ].btns && (
						<div className="bsb-background-video-buttons-wrapper bsb-background-video-buttons-html5">
							<button className="bsb-background-video-play bsb-toggle-video-btn" aria-label={ __( 'Play' ) } aria-hidden="true" style="display: none;"><svg viewBox="0 0 448 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M424.4 214.7L72.4 6.6C43.8-10.3 0 6.1 0 47.9V464c0 37.5 40.7 60.1 72.4 41.3l352-208c31.4-18.5 31.5-64.1 0-82.6z"></path></svg></button>
							<button className="bsb-background-video-pause bsb-toggle-video-btn" aria-label={ __( 'Pause' ) } aria-hidden="false"><svg viewBox="0 0 448 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M144 479H48c-26.5 0-48-21.5-48-48V79c0-26.5 21.5-48 48-48h96c26.5 0 48 21.5 48 48v352c0 26.5-21.5 48-48 48zm304-48V79c0-26.5-21.5-48-48-48h-96c-26.5 0-48 21.5-48 48v352c0 26.5 21.5 48 48 48h96c26.5 0 48-21.5 48-48z"></path></svg></button>
							{ false === bgVideo[ 0 ].mute && (
								<Fragment>
									<button className="bsb-background-video-unmute bsb-toggle-video-btn" aria-label={ __( 'Unmute' ) } aria-hidden="true" style="display: none;"><svg viewBox="0 0 256 512" height="16" width="16" fill="currentColor" xmlns="https://www.w3.org/2000/svg"><path d="M256 88.017v335.964c0 21.438-25.943 31.998-40.971 16.971L126.059 352H24c-13.255 0-24-10.745-24-24V184c0-13.255 10.745-24 24-24h102.059l88.971-88.954c15.01-15.01 40.97-4.49 40.97 16.971z"></path></svg></button>
									<button className="bsb-background-video-mute bsb-toggle-video-btn" aria-label={ __( 'Mute' ) } aria-hidden="false"><svg viewBox="0 0 576 512" height="16" width="16" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M256 88.017v335.964c0 21.438-25.943 31.998-40.971 16.971L126.059 352H24c-13.255 0-24-10.745-24-24V184c0-13.255 10.745-24 24-24h102.059l88.971-88.954c15.01-15.01 40.97-4.49 40.97 16.971zm182.056-77.876C422.982.92 403.283 5.668 394.061 20.745c-9.221 15.077-4.473 34.774 10.604 43.995C468.967 104.063 512 174.983 512 256c0 73.431-36.077 142.292-96.507 184.206-14.522 10.072-18.129 30.01-8.057 44.532 10.076 14.528 30.016 18.126 44.531 8.057C529.633 438.927 576 350.406 576 256c0-103.244-54.579-194.877-137.944-245.859zM480 256c0-68.547-36.15-129.777-91.957-163.901-15.076-9.22-34.774-4.471-43.994 10.607-9.22 15.078-4.471 34.774 10.607 43.994C393.067 170.188 416 211.048 416 256c0 41.964-20.62 81.319-55.158 105.276-14.521 10.073-18.128 30.01-8.056 44.532 6.216 8.96 16.185 13.765 26.322 13.765a31.862 31.862 0 0 0 18.21-5.709C449.091 377.953 480 318.938 480 256zm-96 0c0-33.717-17.186-64.35-45.972-81.944-15.079-9.214-34.775-4.463-43.992 10.616s-4.464 34.775 10.615 43.992C314.263 234.538 320 244.757 320 256a32.056 32.056 0 0 1-13.802 26.332c-14.524 10.069-18.136 30.006-8.067 44.53 10.07 14.525 30.008 18.136 44.53 8.067C368.546 316.983 384 287.478 384 256z"></path></svg></button>
								</Fragment>
							) }
						</div>
					) }
...
const v2 = {
	...
	save: ( { attributes } ) => {
		const { columns, blockAlignment, mobileLayout, currentOverlayTab, overlayBgImg, overlay, colLayout, tabletLayout, collapseOrder, uniqueID, columnGutter, bgColor, bgImg, verticalAlignment, htmlTag, bottomSep, bottomSepColor, topSep, topSepColor, firstColumnWidth } = attributes;
		let bottomSVGDivider;
		if ( 'ct' === bottomSep ) {
			bottomSVGDivider = <path d="M1000,0l-500,98l-500,-98l0,100l1000,0l0,-100Z" />;
		} else if ( 'cti' === bottomSep ) {
		...
		const firstColumnClass = ( firstColumnWidth && columns === 2 ? ' bst-custom-first-width-' + firstColumnWidth : '' );
		const layoutClass = ( ! colLayout ? 'equal' : colLayout );
		const tabLayoutClass = ( ! tabletLayout ? 'inherit' : tabletLayout );
		const HtmlTagOut = ( ! htmlTag ? 'div' : htmlTag );
		const mobileLayoutClass = ( ! mobileLayout ? 'inherit' : mobileLayout );
		...
		const innerClasses = classnames( `bst-row-layout-inner ${ hasBG } bst-layout-id${ classId }` );
		const innerColumnClasses = classnames( `bst-row-column-wrap bst-has-${ columns }-columns bst-gutter-${ columnGutter } bst-row-valign-${ verticalAlignment } bst-row-layout-${ layoutClass } bst-tab-layout-${ tabLayoutClass } bst-m-colapse-${ collapseOrder } bst-mobile-layout-${ mobileLayoutClass }${ firstColumnClass }` );

		return (
			<HtmlTagOut className={ classes }>
				<div id={ `bst-layout-id${ uniqueID }` } className={ innerClasses }>
					{ ( overlay || overlayBgImg ) && (
						<div className={ `bst-row-layout-overlay bst-row-overlay-${ overlayType }` }></div>
					) }
					{ topSep && 'none' !== topSep && '' !== topSep && (
						<div className={ `bst-row-layout-top-sep bst-row-sep-type-${ topSep }` }>
							<svg style={ { fill: topSepColor } } viewBox="0 0 1000 100" preserveAspectRatio="none">
...
const v1 = {
	...
	save: ( { attributes } ) => {
		...
		const classes = classnames( `align${ blockAlignment }` );
		const innerClasses = classnames( `bst-row-layout-inner ${ hasBG } bst-layout-id${ classId }` );
		const innerColumnClasses = classnames( `bst-row-column-wrap bst-has-${ columns }-columns bst-gutter-${ columnGutter } bst-row-valign-${ verticalAlignment } bst-row-layout-${ layoutClass } bst-tab-layout-${ tabLayoutClass } bst-m-colapse-${ collapseOrder } bst-mobile-layout-${ mobileLayoutClass }` );
