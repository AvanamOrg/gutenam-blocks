...
export default function TwoColumnResizer( {
	...
	const onResize = ( event, direction, elt, delta ) => {
		...
		setResizeStyles( `.wp-block-base-rowlayout.bsb-row-id-${ uniqueID } > .innerblocks-wrap.bsb-grid-columns-2.bst-layout-inner-wrap-id${ uniqueID } { grid-template-columns: minmax(0, calc( ${ firstCol }% - (${ currentGutterTotal } / 2 ) ) ) minmax(0, calc( ${ secondCol }% - (${ currentGutterTotal } / 2 ) ) ) !important;}` );
	};
