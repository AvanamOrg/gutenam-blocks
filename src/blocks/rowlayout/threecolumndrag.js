...
export default function ThreeColumnDrag( {
	...
	const onResizeThirds = ( event, direction, elt ) => {
		...
		setResizeStyles( `.wp-block-base-rowlayout.bsb-row-id-${ uniqueID } > .innerblocks-wrap.bsb-grid-columns-3.bst-layout-inner-wrap-id${ uniqueID } { grid-template-columns: minmax(0, calc( ${ tempFirstW }% - (${ currentGutterTotal } / 3 ) ) ) minmax(0, calc( ${ tempSecondW }% - (${ currentGutterTotal } / 3 ) ) )  minmax(0, calc( ${ tempThird }% - (${ currentGutterTotal } / 3 ) ) ) !important;}` );
	};
	...
	const onResizeSecond = ( event, direction, elt ) => {
		...
		//set the temp resized styles on underlying columns
		setResizeStyles( `.wp-block-base-rowlayout.bsb-row-id-${ uniqueID } > .innerblocks-wrap.bsb-grid-columns-3.bst-layout-inner-wrap-id${ uniqueID } { grid-template-columns: minmax(0, calc( ${ tempFirstW }% - (${ currentGutterTotal } / 3 ) ) ) minmax(0, calc( ${ tempSecondWidth }% - (${ currentGutterTotal } / 3 ) ) )  minmax(0, calc( ${ tempThird }% - (${ currentGutterTotal } / 3 ) ) ) !important;}` );
	};
	...
	const innerResizeClasses = classnames( {
		'bst-resizeable-column-container': true,
		[ `bst-resizeable-column-container${ uniqueID }` ]: uniqueID,
	...
	return (
		<div className={ innerResizeClasses } style={ {
			...
			<div className='bsb-inner-resize-row' style={ {
					...
					{ ( { width } ) =>
						<>
							<ResizableBox
								...
								axis="x"
							>
								<span className="editor-row-controls-container base-resize-extra-controls" style={{
									right: isRTL ? undefined : gutterAdjuster,
									left: isRTL ? gutterAdjuster : undefined,
								}}>
									{ columnsUnlocked && (
										<Tooltip text={ __( 'Switch to 5% step resizing', 'gutenam-blocks' ) }>
											<Button
												className="bst-fluid-grid-btn"
												isSmall
									...
									{ ! columnsUnlocked && (
										<Tooltip text={ __( 'Switch to fluid resizing', 'gutenam-blocks' ) }>
											<Button
												className="bst-fluid-grid-btn"
												isSmall
