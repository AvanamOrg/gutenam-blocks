...
import { getPreviewSize } from '@base/helpers';
...
export default function BackendStyles( { id, previewDevice, fieldStyle, labelStyle, helpStyle } ) {
	...
	const fieldCss = (
		<style>
			...
			.wp-block-base-advanced-form_${id} .bsb-advanced-form-field {
				margin-bottom: ${( previewRowGap ? previewRowGap : undefined )};
			}

			.wp-block-base-advanced-form_${id} input:not(.ignore-field-styles),
			.wp-block-base-advanced-form_${id} select:not(.ignore-field-styles),
			.wp-block-base-advanced-form_${id} textarea {
		...
		.wp-block-base-advanced-form_${id} input::placeholder,
		.wp-block-base-advanced-form_${id} select::placeholder,
		.wp-block-base-advanced-form_${id} textarea::placeholder {
			color: ${previewStyles.placeholderColor};
		}

		.wp-block-base-advanced-form_${id} input[type="radio"]{
			border-color: ${previewStyles.borderColor};
		}

		.wp-block-base-advanced-form_${id} input[type="file"]{
			display: block
		}

		.wp-block-base-advanced-form_${id} input[type="checkbox"]{
			border-color: ${previewStyles.borderColor};
		}
	...
	const labelCss = (
		<style>
			...
			.wp-block-base-advanced-form_${id} .bsb-advanced-form-label{
			...
			.wp-block-base-advanced-form_${id} .base-label .required {
	...
	const helpCss = (
		<style>
			...
			.wp-block-base-advanced-form_${id} .bsb-advanced-form-help {
	...
	const submitCss = (
		<style>
			...
			.wp-block-base-advanced-form_${id} .bsb-advanced-form-submit {
