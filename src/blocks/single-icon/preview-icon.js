    ...
    return (
        <div className={`bst-svg-style-${style} bst-svg-icon-wrap bst-svg-item-${uniqueID}`}>
            {icon && (
                <>
                    <IconRender
                        className={`bst-svg-icon bst-svg-icon-${icon}`} name={icon} size={ previewSize }
                        strokeWidth={( 'fe' === icon.substring( 0, 2 ) ? width : undefined )} title={( title ? title : '' )}
                        style={ {
                            color          : ( color ? BaseColorOutput( color ) : undefined ),
                            backgroundColor: ( background && style !== 'default' ? BaseColorOutput( background ) : undefined ),
                            ...
                            borderColor    : ( border && style !== 'default' ? BaseColorOutput( border ) : undefined ),
