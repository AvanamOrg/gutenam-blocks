...
export function Edit ({
	...
	useEffect( () => {
		setBlockDefaults( 'base/show-more', attributes);
	...
	const innerClasses = classnames( {
		'bsb-block-show-more-inner-container': true,
	} );
	const innerBlockProps = useInnerBlocksProps(
		...
		},
		{
            ...
            template: [
				['base/column', {
					className: 'bsb-show-more-content',
				} ],
				['base/advancedbtn', {
					...
					className: 'bsb-show-more-buttons',
				},
					[
						['base/singlebtn', {
							lock: { remove: true, move: true },
							hideLink: true,
							text: __( 'Show More', 'gutenam-blocks' ),
							sizePreset: 'small',
						...
						['base/singlebtn', {
							lock: { remove: true, move: true },
							hideLink: true,
							text: __( 'Show Less', 'gutenam-blocks' ),
							sizePreset: 'small',
							noCustomDefaults: true,
						}]
					]
	...
	const FadeOut = () => {
		...
		return (
			<div className="Class">
				<style>{`
        .bsb-block-show-more-container${ uniqueID } .bsb-show-more-buttons .wp-block-base-singlebtn:last-of-type {
       	display: ${ showHideMore ? 'inline-flex' : 'none' };
       	}

        .bsb-block-show-more-container${ uniqueID } .bsb-show-more-content:not(.is-selected, .has-child-selected) {
	...
	return (
		<Fragment>
			...
			<BaseInspectorControls blockSlug={ 'base/show-more' }>
				<InspectorControlTabs
					panelName={ 'show-more' }
				...
				{( activeTab === 'general' ) &&
					<>
						<BasePanelBody
							title={__( 'Show More Settings', 'gutenam-blocks' )}
							initialOpen={true}
							panelName={ 'showMoreSettings'}
							blockSlug={ 'base/show-more' }
						>
							<ToggleControl
								label={__( 'Display "hide" button once expanded', 'gutenam-blocks' )}
								checked={showHideMore}
								onChange={( value ) => setAttributes( { showHideMore: value } )}
							/>

							<ResponsiveRangeControls
								label={__( 'Maximum Preview Height', 'gutenam-blocks' )}
							...
							<ToggleControl label={__( 'Fade out preview', 'gutenam-blocks' )}
										   checked={enableFadeOut}
										   onChange={( value ) => setAttributes( { enableFadeOut: value } )}/>

							{enableFadeOut && (
								<RangeControl label={__( 'Fade Size', 'gutenam-blocks' )}
											  value={fadeOutSize}
				...
				{( activeTab === 'advanced' ) &&
					<>
						<BasePanelBody panelName={'bsb-show-more-settings'}>
							<ResponsiveMeasureRangeControl
