	...
	componentDidMount() {
		...
		if ( typeof base_blocks_params !== 'undefined' && base_blocks_params.c_fonts ) {
			...
			const custom_fonts = [
				{
					type: 'group',
					label: __( 'Custom Fonts', 'base-custom-fonts' ),
					options: newOptions,
				},
			];
	...
	render() {
		...
		const onTypoFontChange = ( selected ) => {
			if ( selected === null ) {
				...
				let subset;
				if ( selected.google ) {
					if ( typeof base_blocks_params !== 'undefined' && base_blocks_params.g_fonts && ! base_blocks_params.g_fonts[ selected.value ].v.includes( 'regular' ) ) {
						variant = base_blocks_params.g_fonts[ selected.value ].v[ 0 ];
					} else {
						variant = 'regular';
					}
					if ( typeof base_blocks_params !== 'undefined' && base_blocks_params.g_fonts && ! base_blocks_params.g_fonts[ selected.value ].w.includes( 'regular' ) ) {
						weight = base_blocks_params.g_fonts[ selected.value ].w[ 0 ];
					} else {
						weight = '400';
					}
					if ( typeof base_blocks_params !== 'undefined' && base_blocks_params.g_fonts && base_blocks_params.g_fonts[ selected.value ].s.length > 1 ) {
						subset = 'latin';
					} else {
						subset = '';
					}
		...
		// @todo: Replace with icon from @base/icons once created
		let icons = {};
		...
		return (
			<Fragment>
				<Dropdown
					className="bst-popover-font-family-container components-dropdown-menu components-toolbar"
					contentClassName="bst-popover-font-family"
					placement="top"
					renderToggle={ ( { isOpen, onToggle } ) => (
						<Fragment>
							<Button
								className="components-dropdown-menu__toggle bst-font-family-icon"
					...
					renderContent={ () => (
						<Fragment>
							{ onFontFamily && (
								<Fragment>
									<h2 className="bst-heading-fontfamily-title">{ __( 'Font Family' ) }</h2>
									<div className="typography-family-select-form-row block-editor-block-toolbar">
										<Select
											options={ typographyOptions }
											className="bst-inline-typography-select"
											classNamePrefix="bst-typography"
							...
							<div className="typography-row-settings">
								{ onFontWeight && (
									<SelectControl
										label={ __( 'Weight', 'gutenam-blocks' ) }
							...
							<div className="typography-row-settings">
								{ onLetterSpacing && (
									<div className="bst-type-input-wrap">
										<div className="components-base-control bst-typography-number-input">
											<div className="components-base-control__field">
												<label className="components-base-control__label" htmlFor={ `bst-inline-spacing${ uniqueID }` }>{ __( 'Spacing' ) }</label>
												<input
													id={ `bst-inline-spacing${ uniqueID }` }
										...
										<span className="bst-unit">{ __( 'px' ) }</span>
									</div>
								) }
							...
							<div className="typography-row-settings typography-size-row-settings">
								{ onFontSize && onFontSizeType && ! fontSizeArray && (
									<div className="bst-size-input-wrap">
										{ onFontSize && (
											<div className="bst-type-size-input-wrap">
