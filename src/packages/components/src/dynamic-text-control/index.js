	...
	render() {
		...
		return (
			<ToolbarGroup group="dynamic-text">
				<ToolbarButton
					className="bsb-dynamic-menu"
					icon={ icons.dynamic }
					onClick={ () => this.debouncedToggle() }
					isPressed={ false }
					aria-haspopup="true"
					aria-expanded={ open }
					label={  __( 'Dynamic Content', 'gutenam-blocks' ) }
					showTooltip={ true }
				/>
				{ open && (
					<Popover
					...
					>
						<div className="bsb-dynamic-popover-inner-wrap">
							<div className="bsb-pro-notice">
								<h2>{ __( 'Dynamic Content', 'gutenam-blocks-pro' ) } </h2>
								<p>{ __( 'Create dynamic sites by populating content from various sources.', 'gutenam-blocks-pro' ) } </p>
...
export default withFilters( 'base.TextDynamicControl' )( DynamicTextControl );
