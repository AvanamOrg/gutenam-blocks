...
class DynamicBackgroundControl extends Component {
	...
	render() {
		...
		return (
			<div className="bsb-dynamic-background-sidebar-wrap">
				<Button
					className="bsb-dynamic-background-sidebar"
					isTertiary
					icon={ icons.dynamic }
					...
					aria-haspopup="true"
					aria-expanded={ open }
					label={  __( 'Dynamic Background Image', 'gutenam-blocks' ) }
					showTooltip={ true }
				/>
				{ open && (
					<Popover
					...
					>
						<div className="bsb-dynamic-popover-inner-wrap">
							<div className="bsb-pro-notice">
								<h2>{ __( 'Dynamic Background Image', 'gutenam-blocks-pro' ) } </h2>
								<p>{ __( 'Create dynamic sites by populating background images from various sources.', 'gutenam-blocks-pro' ) } </p>
								<ExternalLink href={ 'https://avanam.org/base-blocks/pro/?utm_source=in-app&utm_medium=base-blocks&utm_campaign=dynamic-content' }>{ __( 'Upgrade to Pro', 'gutenam-blocks-pro' ) }</ExternalLink>
							</div>
						</div>
					</Popover>
				) }
...
export default withFilters( 'base.BackgroundDynamicControl' )( DynamicBackgroundControl );
