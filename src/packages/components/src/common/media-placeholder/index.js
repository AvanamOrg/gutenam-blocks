...
class BaseMediaPlaceholder extends Component {
	constructor() {
		super( ...arguments );
		this.state = {
			src: '',
	...
	renderPlaceholder( content, onClick ) {
		...
		if ( ! mediaUpload && ! onSelectURL ) {
			instructions = __( 'To edit this block, you need permission to upload media.', 'gutenam-blocks' );
		}
		...
		if ( instructions === undefined || title === undefined ) {
			...
			if ( instructions === undefined && mediaUpload ) {
				instructions = __( 'Upload a media file or pick one from your media library.', 'gutenam-blocks' );

				if ( isAudio ) {
					instructions = __( 'Upload an audio file, pick one from your media library, or add one with a URL.', 'gutenam-blocks' );
				} else if ( isImage ) {
					instructions = __( 'Upload an image file, pick one from your media library, or add one with a URL.', 'gutenam-blocks' );
				} else if ( isVideo ) {
					instructions = __( 'Upload a video file, pick one from your media library, or add one with a URL.', 'gutenam-blocks' );
				}
			}

			//  if ( title === undefined ) {
			// 	 title = __( 'Media', 'gutenam-blocks' );

			// 	 if ( isAudio ) {
			// 		 title = __( 'Audio', 'gutenam-blocks' );
			// 	 } else if ( isImage ) {
			// 		 title = __( 'Image', 'gutenam-blocks' );
			// 	 } else if ( isVideo ) {
		...
		return (
			onCancel && (
				<Button
					...
					title={ __( 'Cancel', 'gutenam-blocks' ) }
					isLink
					onClick={ onCancel }
				>
		...
		return (
			<div className="block-editor-media-placeholder__url-input-container">
				<Button
					...
					isTertiary
				>
					{ __( 'Insert from URL', 'gutenam-blocks' ) }
				</Button>
				{ isURLInputVisible && (
