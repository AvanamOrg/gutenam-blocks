...
class DynamicLinkControl extends Component {
    ...
    render() {
		...
		return (
			<Fragment>
				<Button
					className="bsb-dynamic-url-sidebar"
					icon={ icons.dynamic }
					onClick={ () => this.debouncedToggle() }
					isPressed={ false }
					aria-haspopup="true"
					aria-expanded={ open }
					label={  __( 'Dynamic Link', 'gutenam-blocks' ) }
					showTooltip={ true }
				/>
				{ open && (
					<Popover
					...
					>
						<div className="bsb-dynamic-popover-inner-wrap">
							<div className="bsb-pro-notice">
								<h2>{ __( 'Dynamic links', 'gutenam-blocks' ) } </h2>
								<p>{ __( 'Create dynamic sites by populating links from various sources.', 'gutenam-blocks' ) } </p>
 ...
 export default withFilters( 'base.URLInputDynamicControl' )( DynamicLinkControl );
