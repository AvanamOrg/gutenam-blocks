	...
	return (
		<div className={ `measure-input-wrap measure-input-${ placement }` }>
			...
			>
				...
				{ isOpen && (
					...
					>
						...
						<ButtonGroup className="bsb-preset-size-options" aria-label={ __( 'Choose a Preset Size', 'gutenam-blocks' ) }>
							{ map( ( preset && Array.isArray( preset ) && preset.length > 1 ? preset : presetSizes[ unit ] ), ( size ) => (
								<Button
									key={ size }
									className="bsb-preset-sizes"
									isSmall
			...
			<TextControl
				...
				placeholder={ ( measurement && 'auto' == measurement ? __( 'auto', 'gutenam-blocks' ) : undefined ) }
				value={ ( undefined !== measurement ? measurement : '' ) }
