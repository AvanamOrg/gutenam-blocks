...
class DynamicGalleryControl extends Component {
    ...
    render() {
		...
		return (
			<div className="bsb-dynamic-image-sidebar-wrap bsb-dynamic-gallery-sidebar-wrap">
				<Button
					className="bsb-dynamic-image-sidebar bsb-dynamic-gallery-sidebar"
					isTertiary
					icon={ icons.dynamic }
					...
					aria-haspopup="true"
					aria-expanded={ open }
					label={  __( 'Dynamic Gallery', 'gutenam-blocks' ) }
					showTooltip={ true }
				/>
				{ open && (
					<Popover
						className="bsb-dynamic-popover"
						...
						ref={ this.popRef }
					>
						<div className="bsb-dynamic-popover-inner-wrap">
							<div className="bsb-pro-notice">
								<h2>{ __( 'Dynamic Gallery', 'gutenam-blocks' ) } </h2>
								<p>{ __( 'Create dynamic sites by populating images from various sources.', 'gutenam-blocks' ) } </p>
								<ExternalLink href={ 'https://avanam.org/base-blocks/pro/?utm_source=in-app&utm_medium=base-blocks&utm_campaign=dynamic-content' }>{ __( 'Upgrade to Pro', 'gutenam-blocks' ) }</ExternalLink>
							</div>
						</div>
					</Popover>
				) }
 ...
 export default withFilters( 'base.GalleryDynamicControl' )( DynamicGalleryControl );
