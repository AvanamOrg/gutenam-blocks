...
class OpacityControl extends Component {
	...
	render() {
		...
		return (
			<div className="bst-popover-container">
				{ this.state.isVisible && (
					<Popover position="top left" className="bst-popover-opacity" onClose={ toggleClose }>
						<RangeControl
				...
				<Tooltip text={ this.props.label }>
					{ ! this.state.isVisible && (
						<Button className="bst-opacity-icon" isSecondary onClick={ toggleVisible }>
							{ icons.opacity }
						</Button>
					) }
					{ this.state.isVisible && (
						<Button className="bst-opacity-icon" isSecondary onClick={ toggleClose }>
							{ icons.opacity }
						</Button>
					) }
				</Tooltip>
