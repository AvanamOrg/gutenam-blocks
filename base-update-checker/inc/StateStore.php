...
if ( !class_exists('Base_Puc_StateStore', false) ):

	class Base_Puc_StateStore {
		/**
		...
		/**
		 * @var Base_Puc_Update|null Cached update.
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Base_Puc_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Base_Puc_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Base_Puc_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Base_Puc_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
